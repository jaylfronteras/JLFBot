...
describe("DeviceRegistry", () => {
  ...
  it("issues a token that authenticates, and never stores it", () => {
    ...
    expect(token.startsWith("jlf_")).toBe(true);
  ...
  it("refuses unknown, empty, and near-miss tokens", () => {
    ...
    expect(registry.authenticate("jlf_nope")).toBeNull();
  ...
  it("replays one logical redemption without creating an orphan device", () => {
    ...
    expect(registry.redeem("jlf_pair_wrong", "iPhone", requestId)).toMatchObject({
      error: expect.stringContaining("no pairing"),
    });
  });
  ...
  it("uses a high-entropy QR credential and burns the manual fallback with it", () => {
    ...
    expect(token).toMatch(/^jlf_pair_[A-Za-z0-9_-]{43}$/);
...
describe("bearerToken", () => {
  it("reads only a well-formed Bearer header", () => {
    expect(bearerToken("Bearer jlf_abc")).toBe("jlf_abc");
    expect(bearerToken("  Bearer jlf_abc  ")).toBe("jlf_abc");
    expect(bearerToken("jlf_abc")).toBeUndefined();
    expect(bearerToken("Basic jlf_abc")).toBeUndefined();
