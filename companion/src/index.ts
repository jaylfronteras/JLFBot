...
parentPort?.on("message", ({ data }) => {
  ...
  if (message.type !== "jlfbot:companion-mutation-token") return;
...
const HARNESS_PORT = num(process.env.JLFBOT_PORT, 8799);
const WEBHOOK_PORT = num(process.env.JLFBOT_WEBHOOK_PORT, HARNESS_PORT + 1);
const COMPANION_PORT = num(process.env.JLFBOT_COMPANION_PORT, 8810);
const CONTROL_PORT = num(process.env.JLFBOT_CONTROL_PORT, 8811);
const SERVICE_TYPE = "_jlfbot._tcp";
let hostedUrl = hostedCompanionUrl(process.env.JLFBOT_COMPANION_HOSTED_URL);
const PRIVATE_ORIGIN = companionOriginSocket(process.env.JLFBOT_COMPANION_INTERNAL_ORIGIN);
const SECRET_PUBLIC_KEY = normalizedPhoneSecretPublicKey(
  process.env.JLFBOT_PHONE_SECRET_PUBLIC_KEY ?? "",
);
...
let cachedName = process.env.JLFBOT_COMPANION_NAME?.trim() || "";

/** What this computer is called on the phone. Never empty. */
const machineName = (): string => cachedName || "JLFBot";
...
async function refreshMachineName(): Promise<void> {
  ...
  try {
    ...
    /* not up, or no profile — "JLFBot" is a fine thing to be called */
  }
}
  ...
  new Promise((resolve, reject) => {
    const onError = (error: NodeJS.ErrnoException) => {
      ...
      const hint = ` — another copy of the companion may already be running; ${
        port === COMPANION_PORT ? "JLFBOT_COMPANION_PORT" : "JLFBOT_CONTROL_PORT"
...
async function main(): Promise<void> {
  const clash =
    conflict("JLFBOT_COMPANION_PORT", COMPANION_PORT) ?? conflict("JLFBOT_CONTROL_PORT", CONTROL_PORT);
  ...
  if (COMPANION_PORT === CONTROL_PORT) {
    throw new Error(
      `JLFBOT_COMPANION_PORT and JLFBOT_CONTROL_PORT are both port ${COMPANION_PORT}, and they cannot share one: ` +
