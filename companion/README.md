...
JLFBot's harness listens on `127.0.0.1` and nothing else, which is the
...
| Environment | Default | |
|---|---|---|
| `JLFBOT_PORT` | `8799` | where the harness is |
| `JLFBOT_WEBHOOK_PORT` | `JLFBOT_PORT` + 1 | the harness's webhook receiver — refused, not used |
| `JLFBOT_COMPANION_PORT` | `8810` | where devices connect |
| `JLFBOT_CONTROL_PORT` | `8811` | the pairing page, loopback only |
| `JLFBOT_COMPANION_DIR` | `~/.jlfbot-companion` | paired devices live here |
| `JLFBOT_COMPANION_NAME` | your name, from the harness | what the phone calls this computer |

`JLFBOT_COMPANION_NAME` overrides a name the sidecar otherwise asks the harness
for at startup — the profile from onboarding, as *"Ada's computer"*. It falls
back to `JLFBot` when the harness is not up or has no profile. Read once
...
it (`JLFBOT_WEBHOOK_PORT`). The companion refuses to start on either and says
