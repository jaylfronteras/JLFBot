...
- **Windows exception: agent-browser 0.36.0-omb.1** is an JLFBot vendor
...
Set `JLFBOT_BROWSER_ARCHIVE_DIR` to a directory of reviewed vendor assets for an
offline build; cached bytes are still verified. Browser dependencies are
updated through reviewed JLFBot releases, not a silent browser updater.

On macOS, release signing applies JLFBot's Developer ID to native
...
imply endorsement of JLFBot. This provenance record is not legal advice
