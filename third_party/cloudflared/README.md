...
JLFBot stages the official `cloudflared` 2026.8.2 executable as a separate
...
| JLFBot target | Release asset | Release asset SHA-256 | Extracted executable SHA-256 |
| --- | --- | --- | --- |
...
checked into git. Set `JLFBOT_CLOUDFLARED_ARCHIVE_DIR` to a directory containing
...
The macOS release process applies JLFBot's Developer ID signature to the
