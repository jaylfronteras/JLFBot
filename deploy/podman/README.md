...
Run the JLFBot server, Caddy, and per-bot Linux desktops with one Podman
...
```text
Browser -> loopback Caddy :8080 -> JLFBot :8799
                                    |
...
```powershell
.\deploy\podman\jlfbot.ps1 setup
# Edit deploy/podman/.env. Set ENGINES to the npm CLI packages you want to use.
.\deploy\podman\jlfbot.ps1 up -d --build
.\deploy\podman\jlfbot.ps1 ps
```

Setup creates/starts a WSL2 machine named `jlfbot` (4 CPUs, 10 GiB RAM,
...
It preserves an existing `.env`. Use `JLFBOT_PODMAN_MACHINE` to select a different
machine. The wrapper refuses a stopped machine for normal Compose commands;
after a reboot, use `podman machine start jlfbot` before `up -d`.

Compose runs **inside** the machine, so the socket and bind paths have the same
meaning for the server and the engine. `JLFBOT_PODMAN_ENV_FILE` selects an alternate
...
for example `jlfbot.ps1 exec jlfbot claude`. Their logins persist under `JLFBOT_DATA_ROOT`.
For the Linux commands, substitute the `podman compose --env-file .env -f
compose.yaml` prefix for `jlfbot.ps1`.
...
The server's `jlf` user (1001) and each desktop's `cua` user (1000) map to the same
...
endpoint to `http://127.0.0.1:8080`, set `JLFBOT_PUBLIC_URL` to that HTTPS URL and
`JLFBOT_HTTPS_HOST` to its hostname, then recreate the services with `up -d`.
Mint a pairing code with `jlfbot.ps1 exec jlfbot node dist-server/jlfbot.js pair`.
...
- Data, chats, credentials, and desktop workspaces live under `JLFBOT_DATA_ROOT`.
