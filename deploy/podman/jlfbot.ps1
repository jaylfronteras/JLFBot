...
if (-not $args.Count -or $args[0] -in @('--help', '-h', 'help')) {
    Write-Output 'Usage: .\deploy\podman\jlfbot.ps1 setup | <compose arguments>'
    Write-Output 'Example: .\deploy\podman\jlfbot.ps1 up -d --build'
    Write-Output 'Options: JLFBOT_PODMAN_MACHINE (default jlfbot), JLFBOT_PODMAN_ENV_FILE (default .env)'
    exit 0
}
...
$machine = if ($env:JLFBOT_PODMAN_MACHINE) { $env:JLFBOT_PODMAN_MACHINE } else { 'jlfbot' }
...
$envFile = if ($env:JLFBOT_PODMAN_ENV_FILE) { $env:JLFBOT_PODMAN_ENV_FILE } else { '.env' }
