...
`JLFBOT_HTTP_PORT` changes the host port and the default public URL.
...
For Tailscale Serve, set `JLFBOT_PUBLIC_URL` to your HTTPS URL and
`JLFBOT_HTTPS_HOST` to its hostname without scheme or path. Configure Tailscale
Serve on the host to forward to the chosen localhost HTTP port.
Keep the default `JLFBOT_BIND_ADDRESS=127.0.0.1`. Caddy refuses to start when
`JLFBOT_HTTPS_HOST` is set with any other bind address, so direct remote HTTP
...
Without `JLFBOT_HTTPS_HOST`, the bind address can be changed for HTTP access.
Only use HTTPS hostname mapping with a trusted local TLS-terminating proxy.

Sign in and pair a browser:

```sh
docker compose exec jlfbot codex login --device-auth
docker compose exec jlfbot node dist-server/jlfbot.js pair
```

On Windows, `./jlfbot.ps1` forwards arguments to Compose using the repository
...
volume, set `JLFBOT_DATA_VOLUME` to its name and `JLFBOT_DATA_EXTERNAL=true`.
