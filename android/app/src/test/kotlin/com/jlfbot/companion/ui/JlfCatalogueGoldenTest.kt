 ...
 * `ios/App/JlfFaceData.swift` for the tables, `ios/App/JlfAvatar.swift` for the
 ...
 */
internal object JlfGolden {

    /** `JlfFaceData.swift:7-48`, in order. */
    ...
    /** `JlfFaceData.swift:397-438`. The head of each pool is the state's resting face. */
    ...
    /** `JlfFaceData.swift:440-481`, in milliseconds. */
    ...
    /** `JlfFaceData.swift:483-507`. A state that is absent never blinks. */
    ...
    /** `JlfFaceData.swift:508-549`, as `field amplitude/period`, in the struct's order. */
    ...
    /** `JlfAvatar.swift:362-365`, head to tail. */
    ...
    /** `JlfFaceData.swift:373-377`: where each expression looks, x then y. */
    ...
    /** `JlfFaceData.swift:379-387`: half-width, curve, gap, skew, per expression. */
    ...
    /**
     * The 4,800 numbers of `JlfFaceData.swift:71-372`, as one number: `h = 0` then
...
/**
 * Is it still the same artwork? `JlfFaceTest` asks whether the face behaves, and
...
class JlfCatalogueGoldenTest {
    ...
    fun `the gaze table is still the desktop's`() {
        for (expression in 0 until 25) {
            assertEquals(JlfGolden.GAZE[expression * 2], JlfFaceData.gazeX(expression), "gaze x $expression")
            assertEquals(JlfGolden.GAZE[expression * 2 + 1], JlfFaceData.gazeY(expression), "gaze y $expression")
        }
    }
    ...
    fun `the mouth table is still the desktop's`() {
        ...
        for (expression in 0 until 25) {
            JlfFaceData.copyMouth(expression, mouth)
            for (part in 0 until 4) {
                assertEquals(JlfGolden.MOUTHS[expression * 4 + part], mouth[part], "mouth $expression part $part")
            }
        }
    }
    ...
    fun `the states are still the desktop's forty, in order`() {
        assertEquals(JlfGolden.STATES, JlfState.entries.map { it.id })
    }

    @Test
    fun `every state still drifts through the desktop's pool`() {
        assertEquals(JlfGolden.POOLS.keys, JlfFaceData.pools.keys.map { it.id }.toSet())
        for (state in JlfState.entries) {
            assertEquals(JlfGolden.POOLS.getValue(state.id), JlfFaceData.pools[state], state.id)
        }
    }

    @Test
    fun `every state still holds an expression for the desktop's span`() {
        assertEquals(JlfGolden.CADENCE.keys, JlfFaceData.expressionCadence.keys.map { it.id }.toSet())
        for (state in JlfState.entries) {
            val expected = JlfGolden.CADENCE.getValue(state.id)
            val actual = JlfFaceData.expressionCadence.getValue(state)
    ...
    fun `the same states still blink, on the same rhythm`() {
        assertEquals(JlfGolden.BLINK.keys, JlfFaceData.blink.keys.map { it.id }.toSet())
        for (state in JlfState.entries) {
            val expected = JlfGolden.BLINK[state.id]
            val actual = JlfFaceData.blink[state]
            if (expected == null) {
    ...
    fun `every body still moves the way the desktop moves it`() {
        for (state in JlfState.entries) {
            assertEquals(
                JlfGolden.MOTION.getValue(state.id),
                render(JlfFaceData.motion.getValue(state)),
                state.id,
            )
        }
    }
    ...
    fun `the comet rings are still the desktop's`() {
        assertEquals(JlfGolden.TRAILS.keys, JlfComets.trails.keys.map { it.id }.toSet())
        for (state in JlfState.entries) {
            val expected = JlfGolden.TRAILS[state.id] ?: continue
            assertEquals(expected, render(JlfComets.trails.getValue(state)), state.id)
        }
    }
    ...
    fun `the face box, its anchor and its mouth stroke are still the desktop's`() {
        // JlfFaceData.swift:63-69
        assertEquals(228.541f, JlfFaceData.FACE_BOX)
        assertEquals(120f, JlfFaceData.FACE_CENTRE_X)
        assertEquals(122.5f, JlfFaceData.FACE_CENTRE_Y)
        assertEquals(7.5f, JlfFaceData.MOUTH_STROKE)
        assertEquals(25, JlfFaceData.EXPRESSION_COUNT)
        assertEquals(48, JlfFaceData.POINTS_PER_RING)
        // JlfAvatar.swift:122 — the viewBox's margin around that box
        assertEquals(15f, JlfSilhouette.FACE_MARGIN)
    }

    @Test
    fun `the face still looks around by the desktop's fraction`() {
        // JlfAvatar.swift:277
        assertEquals(0.35f, JlfFaceEngine.LOOK_AROUND)
    }

    @Test
    fun `a blink still takes 320ms, shut at 42 percent of it`() {
        // JlfAvatar.swift:611-617, landing on idle's earliest blink — 6000ms
        val engine = JlfFaceEngine(AT_THE_EARLIEST)
        engine.setState(JlfState.IDLE, 0L)
        engine.advance(0L)
    ...
    fun `the morph still springs at the desktop's stiffness`() {
        ...
        val engine = JlfFaceEngine()
        engine.setState(JlfState.IDLE, 0L)
        engine.advance(0L)
        engine.setState(JlfState.LAUGHING, 0L)
    ...
    fun `a clock that jumped is still worth at most 100ms of spring`() {
        // JlfAvatar.swift:313
        val jumped = JlfFaceEngine()
        jumped.setState(JlfState.IDLE, 0L)
        jumped.advance(0L)
        jumped.setState(JlfState.LAUGHING, 0L)
        jumped.advance(millis(10_000f))

        val stepped = JlfFaceEngine()
        stepped.setState(JlfState.IDLE, 0L)
        stepped.advance(0L)
        stepped.setState(JlfState.LAUGHING, 0L)
    ...
    private fun progress(engine: JlfFaceEngine): Float = (engine.mouth()[0] - 12f) / (25f - 12f)
    ...
    private fun render(motion: JlfBodyMotion): String {
    ...
    private fun render(spec: JlfCometSpec): String =
