 ...
 * `JlfCatalogueGoldenTest` is where it is asked.
 */
class JlfFaceDataTest {

    @Test
    fun `every state has a pool, a cadence and a motion`() {
        assertEquals(40, JlfState.entries.size)
        for (state in JlfState.entries) {
            assertTrue(JlfFaceData.pools.containsKey(state), "pool ${state.id}")
            assertTrue(JlfFaceData.expressionCadence.containsKey(state), "cadence ${state.id}")
            assertTrue(JlfFaceData.motion.containsKey(state), "motion ${state.id}")
        }
    }

    @Test
    fun `every pool points into the catalogue and starts with a resting face`() {
        for ((state, pool) in JlfFaceData.pools) {
            assertTrue(pool.isNotEmpty(), "empty pool ${state.id}")
            for (expression in pool) {
                assertTrue(expression in 0 until JlfFaceData.EXPRESSION_COUNT, "${state.id} → $expression")
            }
        }
    }
    ...
    fun `a cadence and a blink rhythm are a span, not a point going backwards`() {
        for ((state, range) in JlfFaceData.expressionCadence) {
            assertTrue(range.minMillis > 0f && range.maxMillis >= range.minMillis, state.id)
        }
        for ((state, range) in JlfFaceData.blink) {
            assertTrue(range.minMillis > 0f && range.maxMillis >= range.minMillis, state.id)
        }
    }
    ...
    fun `every expression is two rings of 48 points around the face centre`() {
        val ring = FloatArray(JlfFaceData.RING_STRIDE)
        for (expression in 0 until JlfFaceData.EXPRESSION_COUNT) {
            for (eye in 0..1) {
                ...
                assertTrue(minX >= 0f && maxX <= JlfFaceData.FACE_BOX, "ring $expression/$eye off the face")
                assertTrue(minY >= 0f && maxY <= JlfFaceData.FACE_BOX, "ring $expression/$eye off the face")
            }
        }
        // the first ring is the left eye, in every expression
        val left = FloatArray(JlfFaceData.RING_STRIDE)
        val right = FloatArray(JlfFaceData.RING_STRIDE)
        for (expression in 0 until JlfFaceData.EXPRESSION_COUNT) {
            JlfFaceData.copyRing(expression, 0, left)
            JlfFaceData.copyRing(expression, 1, right)
    ...
    fun `every expression has a mouth wide enough to see, below the eyes`() {
        val mouth = FloatArray(JlfFaceData.MOUTH_STRIDE)
        for (expression in 0 until JlfFaceData.EXPRESSION_COUNT) {
            JlfFaceData.copyMouth(expression, mouth)
            assertTrue(mouth[JlfFaceData.MOUTH_HALF_WIDTH] > 0f, "no mouth in $expression")
            assertTrue(mouth[JlfFaceData.MOUTH_GAP] > 0f, "mouth on the eyes in $expression")
        }
    }
}
 ...
 *
 * Which face that is comes from [JlfGolden], not from the pools being tested.
 */
class JlfFaceEngineTest {
    private val ring = FloatArray(JlfFaceData.RING_STRIDE)
    private val mouth = FloatArray(JlfFaceData.MOUTH_STRIDE)
    ...
    fun `a face that is never advanced rests on the desktop's face for its state`() {
        for (state in JlfState.entries) {
            ...
            val expression = JlfGolden.RESTING.getValue(state.id)

            for (eye in 0..1) {
                JlfFaceData.copyRing(expression, eye, ring)
                val offsetX = JlfGolden.GAZE[expression * 2] * LOOK_AROUND
                val offsetY = JlfGolden.GAZE[expression * 2 + 1] * LOOK_AROUND
            ...
            for (index in mouth.indices) {
                assertEquals(JlfGolden.MOUTHS[expression * 4 + index], engine.mouth()[index], 0.001f, state.id)
            }
    ...
    fun `the morph settles on the face of the state it was handed`() {
        ...
        val target = JlfGolden.RESTING.getValue(JlfState.LAUGHING.id)
        JlfFaceData.copyRing(target, 0, ring)
        ...
        val offsetX = JlfGolden.GAZE[target * 2] * LOOK_AROUND
    ...
    fun `a face that stops being advanced settles on the state it was handed`() {
        ...
        engine.setState(JlfState.IDLE, 0L)
        engine.advance(0L)
        engine.setState(JlfState.ANGRY, 0L)
        engine.advance(FRAME_NANOS)
        engine.rest()

        val target = JlfGolden.RESTING.getValue(JlfState.ANGRY.id)
        JlfFaceData.copyRing(target, 0, ring)
        val offsetX = JlfGolden.GAZE[target * 2] * LOOK_AROUND
    ...
    fun `a state with no blink rhythm never blinks`() {
        val engine = JlfFaceEngine()
        engine.setState(JlfState.SLEEPING, 0L)
 ...
 */
class JlfBodyPoseTest {
    private val pose = JlfBodyPose()

    @Test
    fun `a state with no motion leaves the body where it stands`() {
        JlfFaceData.motion.getValue(JlfState.THINKING_DOTS).poseAt(1234f, pose)
    ...
    fun `a tilt is held for as long as the state is`() {
        val dragging = JlfFaceData.motion.getValue(JlfState.DRAGGING)
    ...
    fun `a bob lifts the body, and squashes it as it lands`() {
        ...
        val bouncing = JlfFaceData.motion.getValue(JlfState.BOUNCING)
    ...
    fun `an entrance grows to full size and then stops growing`() {
        ...
        val spawning = JlfFaceData.motion.getValue(JlfState.SPAWNING)
 ...
 * reading as an orbit.
 */
class JlfCometFieldTest {
    @Test
    fun `a comet ring passes behind the body and in front of it`() {
        val field = JlfCometField()
        val spec = JlfComets.trails.getValue(JlfState.ORBIT)
        var behind = 0
        var front = 0
    ...
    fun `every piece is an outline that fits its buffer`() {
        val field = JlfCometField()
        for (spec in JlfComets.trails.values) {
            for (frame in 0 until 30) {
                ...
                for (index in 0 until field.count) {
                    ...
                    assertTrue(piece.colorIndex in JlfComets.colors.indices)
    ...
    fun `an empty frame leaves nothing behind from the last one`() {
        val field = JlfCometField()
        field.update(JlfComets.trails.getValue(JlfState.ORBIT), 0f, CENTRE, CENTRE)
        assertTrue(field.count > 0)
        field.update(JlfComets.trails.getValue(JlfState.RADAR).copy(count = 0), 0f, CENTRE, CENTRE)
        assertEquals(0, field.count)
    }

    private companion object {
        const val CENTRE = JlfFaceData.FACE_BOX / 2f
    }
}
