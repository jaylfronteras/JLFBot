...
/**
 * The mascot's body, as geometry: the per-body cache that turns [JlfBodies] into
 * something a canvas can fill — the port of `JlfSilhouette` in
 * `ios/Sources/CompanionCore/JlfSilhouette.swift`.
 *
 ...
 * avatar is. The face the body wears is [JlfFaceData]; this is only the shape it is
 * painted on.
 */
internal object JlfSilhouette {
     ...
     * [JlfFaceData.FACE_BOX], which is the same box seen from the artwork's side.
     */
    const val FACE_BOX: Float = JlfFaceData.FACE_BOX

    /** The shipped mascot, and what anything unrecognised falls back to. */
    val defaultBody: String get() = JlfBodies.DEFAULT_ID
