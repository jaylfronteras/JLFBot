 ...
 * desktop. The port of `JlfAvatar` in `ios/App/JlfAvatar.swift`: [JlfFaceEngine]
 ...
 */
@Composable
internal fun JlfAvatar(
    color: String,
    /** Which catalog body to draw; null and unknown ids fall back to the cursor. */
    bodyId: String? = JlfSilhouette.defaultBody,
    size: Dp = 52.dp,
    state: JlfState = JlfState.IDLE,
    /** Off draws the state's resting face, still. For lists of many. */
    animated: Boolean = true,
    modifier: Modifier = Modifier,
) {
    val engine = remember { JlfFaceEngine() }
    val geometry = remember { JlfFaceGeometry() }
    val clock = remember { JlfFrameClock() }
    val body = remember(color, bodyId) {
        // The gradient runs corner to corner of the body's own bounds.
        val bounds = JlfSilhouette.faceBoxBounds(bodyId)
        Brush.linearGradient(
            colorStops = JlfPalette.gradient(color)
    ...
    Canvas(
        ...
        drawJlf(engine, geometry, body, bodyId, live, now)
    }
}
...
fun ProfileAvatar(name: String, size: Dp = 34.dp, modifier: Modifier = Modifier) {
    Box(modifier = modifier.size(size), contentAlignment = Alignment.Center) {
        Canvas(Modifier.size(size)) {
            drawCircle(color = Color(JlfPalette.argb("green")))
        }
        Text(
...
private class JlfFaceGeometry {
    val eyes: Array<Path> = arrayOf(Path(), Path())
    val mouth: Path = Path()
    val comet: Path = Path()
    val comets: JlfCometField = JlfCometField()
    val pose: JlfBodyPose = JlfBodyPose()
    ...
    val cometBrushes: Array<Brush> by lazy(LazyThreadSafetyMode.NONE) {
        Array(JlfComets.colors.size) { index ->
            val palette = JlfComets.colors[index]
            Brush.linearGradient(
                colorStops = arrayOf(
...
private val mouthStroke = Stroke(width = JlfFaceData.MOUTH_STROKE, cap = StrokeCap.Round)

private fun DrawScope.drawJlf(
    engine: JlfFaceEngine,
    geometry: JlfFaceGeometry,
    body: Brush,
    bodyId: String?,
    live: Boolean,
    nowNanos: Long,
) {
    val box = JlfFaceData.FACE_BOX + JlfSilhouette.FACE_MARGIN * 2
    val fit = min(size.width, size.height) / box
    withTransform({
        translate((size.width - box * fit) / 2f, (size.height - box * fit) / 2f)
        scale(fit, fit, Offset.Zero)
        translate(JlfSilhouette.FACE_MARGIN, JlfSilhouette.FACE_MARGIN)
    }) {
        val centre = JlfFaceData.FACE_BOX / 2f
        val elapsed = engine.elapsedMillis(nowNanos)
        val trail = JlfComets.trails[engine.state]
        if (trail != null) {
        ...
        withTransform({
            ...
            if (live) {
                ...
                scale(pose.squashX, pose.squashY, Offset(centre, JlfFaceData.FACE_BOX))
            }
        }) {
...
private fun DrawScope.drawBody(
    ...
    // Parsed and placed once per body, inside [JlfSilhouette]; this is a lookup.
    val silhouette = JlfSilhouette.inFaceBox(bodyId)
    val anchor = JlfSilhouette.anchor(bodyId)
    ...
    clipPath(silhouette) {
        withTransform({
            ...
            translate(-JlfFaceData.FACE_CENTRE_X, -JlfFaceData.FACE_CENTRE_Y)
        }) {
...
private fun DrawScope.drawMouth(left: FloatArray, right: FloatArray, mouth: FloatArray, path: Path) {
    ...
    val drop = halfHeight + mouth[JlfFaceData.MOUTH_GAP]
    val originX = (leftX + rightX) / 2f - sin(theta) * drop
    val originY = (leftY + rightY) / 2f + cos(theta) * drop
    val angle = theta + mouth[JlfFaceData.MOUTH_SKEW] * DEGREES_TO_RADIANS
    val cosAngle = cos(angle)
    val sinAngle = sin(angle)
    val halfWidth = mouth[JlfFaceData.MOUTH_HALF_WIDTH]
    val curve = mouth[JlfFaceData.MOUTH_CURVE]
 ...
 */
private fun DrawScope.drawComets(geometry: JlfFaceGeometry, front: Boolean) {
