...
import com.jlfbot.companion.core.RoutineInput
...
object RoutineRules {
    ...
    const val UNKNOWN_SCHEDULE_NOTE: String =
        "This routine uses a schedule added by a newer JLFBot. Choose One time, " +
    ...
    const val CLOUD_READY_FOOTER: String =
        "Runs the agent and its tools inside its Box virtual machine. The VM wakes " +
            "automatically for each run; keep JLFBot running so its scheduler can " +
            "launch the job."
    ...
    fun locationLabel(location: RoutineRunLocation): String = when (location) {
        RoutineRunLocation.JLF -> "This computer"
    ...
    fun locationFooter(
        ...
        runOn == RoutineRunLocation.JLF -> JLF_FOOTER
