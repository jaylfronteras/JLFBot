...
internal fun browseDiscoveryFlow(
    ...
    fun newListener(): NsdManager.DiscoveryListener = object : NsdManager.DiscoveryListener {
        ...
        override fun onServiceFound(service: NsdServiceInfo) {
            if (service.serviceType != serviceType && !service.serviceType.contains("jlfbot")) {
                return
            }
            attemptResolve(service, attemptsSoFar = 1)
        }
...
/**
 * NsdManager wrapper for `_jlfbot._tcp`, exposed as a Flow of [DiscoveryState].
 */
class NsdDiscovery(
    context: Context,
    ...
    companion object {
        const val SERVICE_TYPE = "_jlfbot._tcp."
        const val MULTICAST_LOCK_TAG = "jlfbot-nsd"
    }
}
