...
/**
 * The face, frame by frame — the port of `JlfFaceEngine` in
 * `ios/App/JlfAvatar.swift`, which is itself the desktop's frame loop: a state
 ...
 */
internal class JlfFaceEngine(private val random: Random = Random.Default) {
    var state: JlfState = JlfState.IDLE
        private set

    var motion: JlfBodyMotion = JlfBodyMotion()
        private set

    private var expression = 0
    private val currentRings = Array(EYES) { FloatArray(JlfFaceData.RING_STRIDE) }
    private val targetRings = Array(EYES) { FloatArray(JlfFaceData.RING_STRIDE) }
    private val displayedRings = Array(EYES) { FloatArray(JlfFaceData.RING_STRIDE) }
    private val currentMouth = FloatArray(JlfFaceData.MOUTH_STRIDE)
    private val targetMouth = FloatArray(JlfFaceData.MOUTH_STRIDE)
    private val displayedMouth = FloatArray(JlfFaceData.MOUTH_STRIDE)
    private var currentGazeX = JlfFaceData.gazeX(0)
    private var currentGazeY = JlfFaceData.gazeY(0)
    ...
    init {
        for (eye in 0 until EYES) {
            JlfFaceData.copyRing(0, eye, currentRings[eye])
            JlfFaceData.copyRing(0, eye, targetRings[eye])
        }
        JlfFaceData.copyMouth(0, currentMouth)
        JlfFaceData.copyMouth(0, targetMouth)
    }

    fun setState(next: JlfState, nowNanos: Long) {
        if (started && next == state) return
        started = true
        state = next
        stateStartNanos = nowNanos
        motion = JlfFaceData.motion[next] ?: JlfBodyMotion()
        select(JlfFaceData.pools[next]?.firstOrNull() ?: 0)
        // first frame: rest on it, no morph in
        if (lastNanos == UNSET) {
            morph = 1f
            velocity = 0f
        }
        nextExpressionAtNanos = schedule(JlfFaceData.expressionCadence[next], nowNanos)
        nextBlinkAtNanos = schedule(JlfFaceData.blink[next], nowNanos)
    }
    ...
    fun advance(nowNanos: Long) {
        ...
        if (nextExpressionAtNanos != UNSET && nowNanos >= nextExpressionAtNanos) {
            ...
            nextExpressionAtNanos = schedule(JlfFaceData.expressionCadence[state], nowNanos)
        }
        if (nextBlinkAtNanos != UNSET && nowNanos >= nextBlinkAtNanos) {
            blinkStartNanos = nowNanos
            nextBlinkAtNanos = schedule(JlfFaceData.blink[state], nowNanos)
        }
    }
    ...
    private fun select(index: Int) {
        val count = JlfFaceData.EXPRESSION_COUNT
        ...
        for (eye in 0 until EYES) {
            ...
            JlfFaceData.copyRing(next, eye, target)
        }
        for (i in currentMouth.indices) currentMouth[i] += (targetMouth[i] - currentMouth[i]) * m
        JlfFaceData.copyMouth(next, targetMouth)
        currentGazeX = gazeX()
        currentGazeY = gazeY()
        targetGazeX = JlfFaceData.gazeX(next)
        targetGazeY = JlfFaceData.gazeY(next)
    ...
    private fun nextExpression(): Int {
        val pool = JlfFaceData.pools[state] ?: return 0
        var alternatives = 0
    ...
    private fun schedule(range: JlfRange?, nowNanos: Long): Long {
 ...
 */
internal fun JlfBodyMotion.poseAt(elapsedMillis: Float, out: JlfBodyPose) {
    out.dx = 0f
    out.dy = 0f
