...
class PairingClientTest {
    ...
    fun retryRecoversLostResponseWithSameLogicalRequestId() = runBlocking {
        ...
        assertEquals("jlf_device", outcome.response.token)
        assertEquals(listOf(requestId, requestId), stub.pairRequests.map { it.stringBody("pairRequestId") })
    }
    ...
    private companion object {
        const val CREDENTIAL = "jlf_pair_aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
        const val HEALTH = """{"app":"jlfbot","pid":42,"static":true}"""
        const val PAIRED =
            """{"token":"jlf_device","device":{"id":"d","name":"Pixel","createdAt":1,"lastSeenAt":1},"serverName":"Mac","hosts":["192.168.1.42"]}"""
    }
}
