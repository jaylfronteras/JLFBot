package com.jlfbot.companion.core
...
class ConnectionTest {
    ...
    fun parsersNormalizeNonIpv6ZoneSuffixes() {
        ...
        val invite = PairingInvite.parse(
            URI("jlfbot://pair?address=192.168.1.3%25en0%3A9910&code=004209"),
        )
    ...
    fun parsesADesktopPairingInvite() {
        val token = "jlf_pair_" + "a".repeat(43)
        val invite = PairingInvite.parse(
            URI("jlfbot://pair?address=macbook.tail1234.ts.net%3A8810&token=$token&code=004209&name=Milind%27s%20Mac"),
        )!!
    ...
    fun literalPlusInPairingInviteNameIsPreserved() {
        val invite = PairingInvite.parse(
            URI("jlfbot://pair?address=mac.local&code=004209&name=Ada%27s+Mac"),
        )
    ...
    fun parsesAnOlderCodeOnlyPairingInvite() {
        val invite = PairingInvite.parse(URI("jlfbot://pair?address=mac.local&code=004209"))
    ...
    fun typedInviteEstablishesPolicyFromItsSelectedEndpoint() {
        ...
        val invite = assertNotNull(PairingInvite.parse(URI(
            "jlfbot://pair?address=192.168.1.42%3A8810&token=$token&endpoints=${base64Url(routes)}",
        )))
    ...
    fun presentButInvalidTypedEndpointsRejectTheWholeInvite() {
        ...
        assertNull(PairingInvite.parse(URI(
            "jlfbot://pair?address=192.168.1.42%3A8810&token=$token&endpoints=",
        )))
        assertNull(PairingInvite.parse(URI(
            "jlfbot://pair?address=192.168.1.42%3A8810&token=$token&endpoints=${"a".repeat(8_193)}",
        )))
    }
    ...
    fun dropsUnusableFallbackHostsWithoutRefusingTheInvite() {
        val invite = PairingInvite.parse(URI(
            "jlfbot://pair?address=mac.local&code=004209&hosts=%20192.168.1.42%20,,bad%2Fslash,has%20space",
        ))!!
        assertEquals(emptyList(), invite.connection.hosts)
        val empty = PairingInvite.parse(
            URI("jlfbot://pair?address=mac.local&code=004209&hosts=bad%2Fslash"),
        )
    ...
    fun pairResponseWithAndWithoutHostsDecodes() {
        val older = CompanionJson.decodeFromString<PairResponse>(
            """{"token":"jlf_x","device":{"id":"d","name":"p","createdAt":1,"lastSeenAt":1},"serverName":"Mac"}""",
        )
        assertNull(older.hosts)
        val newer = CompanionJson.decodeFromString<PairResponse>(
            """{"token":"jlf_x","device":{"id":"d","name":"p","createdAt":1,"lastSeenAt":1},"serverName":"Mac","hosts":["a.ts.net","192.168.1.42"]}""",
        )
        assertEquals(listOf("a.ts.net", "192.168.1.42"), newer.hosts)
        val typed = CompanionJson.decodeFromString<PairResponse>(
            """{"token":"jlf_x","device":{"id":"d","name":"p","createdAt":1,"lastSeenAt":1},"serverName":"Mac","endpoints":[{"url":"https://mac.example","kind":"hosted","priority":0}]}""",
        )
    ...
    fun rejectsUntrustedOrMalformedPairingInvites() {
        listOf(
            ...
            "jlfbot://pair?address=mac.local&code=12345",
            "jlfbot://pair?address=mac.local&token=weak",
            "jlfbot://pair?address=mac.local&token=weak&code=123456",
            "jlfbot://pair?address=host%2Fpath&code=123456",
            "jlfbot://pair?address=one.local&address=two.local&code=123456",
