...
data class PairingInvite(val connection: Connection, val credential: String) {
    companion object {
        ...
        fun parse(url: URI): PairingInvite? {
            if (!url.scheme.equals("jlfbot", ignoreCase = true) ||
        ...
        private fun credential(values: Map<String, String>): String? {
            values["token"]?.let { token ->
                val suffix = token.removePrefix("jlf_pair_")
                if (!token.startsWith("jlf_pair_") || token.toByteArray().size != 52 || suffix.length != 43) {
                    return null
                }
