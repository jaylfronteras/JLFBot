...
//
// The body is whichever one the bot wears: `JlfBodies` in `CompanionCore` is
...
// placing a body is `CompanionCore.JlfSilhouette`; this file is the drawing.
//
...
// `JlfFaceEngine` — same tables, same numbers, same face.
import CompanionCore
import SwiftUI

enum JlfPalette {
    /// src/lib/mascot.ts — JLF_COLORS
...
/// to `CompanionCore.JlfSilhouette`, where `swift test` can reach it. What
/// stays here is what depends on `JlfFaceData`, which is app-side artwork.
extension JlfSilhouette {
    ...
    static func fit(_ rect: CGRect) -> CGAffineTransform {
        let side = JlfFaceData.faceBox + margin * 2
...
/// on the desktop. `JlfFaceEngine` is a port of the frame loop in
/// `JlfMascotios/JlfAvatar.tsx`: a state picks a pool of expressions and
...
struct JlfAvatar: View {
    ...
    var bodyId: String? = JlfSilhouette.defaultBody
    var state: JlfState = .idle
    ...
    @State private var engine = JlfFaceEngine()
...
final class JlfFaceEngine {
    private(set) var state: JlfState = .idle
    private var expression = 0
    private var currentRings: [[CGPoint]] = [JlfFaceData.ring(0, eye: 0), JlfFaceData.ring(0, eye: 1)]
    private var targetRings: [[CGPoint]] = [JlfFaceData.ring(0, eye: 0), JlfFaceData.ring(0, eye: 1)]
    private var currentMouth = JlfFaceData.mouth(0)
    private var targetMouth = JlfFaceData.mouth(0)
    private var currentGaze = JlfFaceData.gaze(0)
    private var targetGaze = JlfFaceData.gaze(0)
    ...
    func setState(_ new: JlfState, now: Date) {
        guard !started || new != state else { return }
        started = true
        state = new
        stateStart = now
        select(JlfFaceData.pools[new]?.first ?? 0)
        if last == nil { morph = 1; velocity = 0 } // first frame: rest on it, no morph in
        nextExpressionAt = schedule(JlfFaceData.expressionCadence[new], from: now)
        nextBlinkAt = schedule(JlfFaceData.blink[new], from: now)
    }
    ...
    private func select(_ index: Int) {
        let i = ((index % JlfFaceData.expressionCount) + JlfFaceData.expressionCount) % JlfFaceData.expressionCount
        if i == expression && morph >= 1 { return }
        currentRings = displayedRings()
        currentMouth = displayedMouth()
        currentGaze = displayedGaze()
        targetRings = [JlfFaceData.ring(i, eye: 0), JlfFaceData.ring(i, eye: 1)]
        targetMouth = JlfFaceData.mouth(i)
        targetGaze = JlfFaceData.gaze(i)
        expression = i
        morph = 0
        velocity = 0
    }
    ...
    func step(now: Date) {
        ...
        if let due = nextExpressionAt, now >= due {
            let pool = JlfFaceData.pools[state] ?? [0]
            let alternatives = pool.filter { $0 != expression }
            select(alternatives.randomElement() ?? pool[0])
            nextExpressionAt = schedule(JlfFaceData.expressionCadence[state], from: now)
        }
        if let due = nextBlinkAt, now >= due {
            blinkStart = now
            nextBlinkAt = schedule(JlfFaceData.blink[state], from: now)
        }
    }
    ...
    static let cometTrails: [JlfState: CometSpec] = [
    ...
    /// for a still frame.
    func draw(in context: inout GraphicsContext, size: CGSize, color: String, bodyId: String? = JlfSilhouette.defaultBody, bodyMotion: Bool, comets: Bool = false, at now: Date = Date()) {
        let rect = CGRect(origin: .zero, size: size)
        context.concatenate(JlfSilhouette.fit(rect))
        ...
        let centre = CGPoint(x: JlfFaceData.faceBox / 2, y: JlfFaceData.faceBox / 2)
        ...
        if bodyMotion {
            bodyContext.concatenate(bodyTransform(JlfFaceData.motion[state] ?? JlfBodyMotion(), elapsed: elapsed))
        }
    ...
    private func drawBody(in context: inout GraphicsContext, color: String, bodyId: String?, now: Date) {
        ...
        let body = Path(JlfSilhouette.inFaceBox(bodyId))
        let a = JlfSilhouette.anchor(bodyId)

        // The gradient runs corner to corner of the body's own bounds,
        // which is the only place that lookup is still needed.
        let bounds = JlfSilhouette.faceBoxBounds(bodyId)
        context.fill(body, with: .linearGradient(
            Gradient(stops: JlfPalette.gradientStops(color)),
        ...
        context.translateBy(x: -JlfFaceData.faceCentre.x, y: -JlfFaceData.faceCentre.y)
        ...
        context.stroke(
            ...
            with: .color(.white),
            style: StrokeStyle(lineWidth: JlfFaceData.mouthStroke, lineCap: .round)
        )
    }

    /// The desktop's `bodyTransform`, in face-box units, elapsed in ms.
    private func bodyTransform(_ m: JlfBodyMotion, elapsed: CGFloat) -> CGAffineTransform {
        let centre = JlfFaceData.faceBox / 2
        let ground = JlfFaceData.faceBox
