...
enum Keychain {
    static func save(_ token: String, for connectionId: String) throws {
        try JLFBotSharedKeychain.save(token, for: connectionId)
    }
    ...
    static func token(for connectionId: String) throws -> String? {
        try JLFBotSharedKeychain.tokenMigratingLegacyItem(for: connectionId)
    }

    @discardableResult
    static func remove(_ connectionId: String) -> Bool {
        JLFBotSharedKeychain.removeIncludingLegacyItem(connectionId)
    }
}

typealias KeychainError = JLFBotSharedKeychainError
