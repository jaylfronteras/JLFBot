...
// that turns `JlfBodies` into something a canvas can fill.
//
...
// to sit in `ios/App/JlfAvatar.swift` with a single hardcoded path to get
...
// phone knows that is *not* a view (see `ios/Package.swift`). `JlfAvatar`
...
public enum JlfSilhouette {
    ...
    /// in. Mirrors `JlfFaceData.faceBox`, which is the same box seen from
    /// the artwork's side.
    public static let faceBox: CGFloat = 228.541

    /// The shipped mascot, and what anything unrecognised falls back to.
    /// `JlfBodies` itself is generated and stays internal to this module;
    /// the app reaches the catalog through this type.
    public static let defaultBody: String = JlfBodies.defaultID
    ...
    public static func anchor(_ id: String?) -> (x: CGFloat, y: CGFloat, scale: CGFloat) {
        JlfBodies.body(id).anchor
    }

    private static func placed(_ id: String?) -> Placed {
        let body = JlfBodies.body(id)
        lock.lock()
        defer { lock.unlock() }
