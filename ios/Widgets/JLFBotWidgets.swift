...
@main
struct JLFBotWidgets: WidgetBundle {
    ...
    var body: some WidgetConfiguration {
        ActivityConfiguration(for: BotActivityAttributes.self) { context in
            ...
            DynamicIsland {
                ...
                JlfFaceStill(color: context.attributes.color, state: JlfState(rawValue: context.state.face) ?? .idle, size: 24)
            } compactTrailing: {
                compactTrailing(context)
            } minimal: {
                JlfFaceStill(color: context.attributes.color, state: JlfState(rawValue: context.state.face) ?? .idle, size: 22)
            }
            .keylineTint(JlfPalette.color(context.attributes.color))
        }
    }
    ...
    private func compactTrailing(_ context: ActivityViewContext<BotActivityAttributes>) -> some View {
        ...
        case "needsYou":
            Image(systemName: "hand.raised.fill")
                ...
                .foregroundStyle(JlfPalette.color(context.attributes.color))
        case "working":
            Image(systemName: "circle.dotted")
                .font(.system(size: 13, weight: .semibold))
                .foregroundStyle(.white.opacity(0.8))
        default:
            Circle().fill(JlfPalette.color(context.attributes.color)).frame(width: 8, height: 8)
        }
    }
}
                        ...
                        .background(
                            Capsule().fill(
                                option.caseInsensitiveCompare("Deny") == .orderedSame
                                    ...
                                    : JlfPalette.color(context.attributes.color)
                            )
                        )
                }
                .buttonStyle(.plain)
...
private struct OrbitingFace: View {
    ...
    var body: some View {
        ZStack {
            ...
            JlfFaceStill(color: context.attributes.color, state: JlfState(rawValue: context.state.face) ?? .idle, size: size, comets: true, at: Date())
        }
    }
}
