...
enum JLFBotConnectionRegistrySource: Equatable {
    case sharedRegistry
...
struct JLFBotConnectionRegistryResolution: Equatable {
    let registry: CompanionConnectionRegistry
    let source: JLFBotConnectionRegistrySource
}

/// Non-secret pairing metadata shared with extensions.
///
/// Tokens never enter defaults; they live in `JLFBotSharedKeychain`. The
...
enum JLFBotSharedConnectionStore {
    ...
    static func resolve(
    ...
    ) -> JLFBotConnectionRegistryResolution {
        ...
        if let sharedRegistryData,
           let registry = try? decoder.decode(
            ...
            return JLFBotConnectionRegistryResolution(
                registry: registry,
    ...
    static func loadRegistry(
        sharedDefaults: UserDefaults? = JLFBotSharedConfiguration.sharedDefaults,
