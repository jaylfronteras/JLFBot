...
enum JLFBotSharedKeychain {
    private static let service = "com.jlfbot.companion.token"
    ...
    static func remove(_ connectionID: String) -> Bool {
        guard let accessGroup = JLFBotSharedConfiguration.keychainAccessGroup else {
            return false
        }
    ...
    static func removeIncludingLegacyItem(_ connectionID: String) -> Bool {
        ...
        guard let legacyGroup = JLFBotSharedConfiguration.legacyAppKeychainAccessGroup else {
            return false
        }
    ...
    private static func requiredAccessGroup() throws -> String {
        guard let accessGroup = JLFBotSharedConfiguration.keychainAccessGroup else {
            throw JLFBotSharedKeychainError.configurationMissing
        }
        return accessGroup
    }

    private static func requiredLegacyAccessGroup() throws -> String {
        guard let accessGroup = JLFBotSharedConfiguration.legacyAppKeychainAccessGroup else {
            throw JLFBotSharedKeychainError.configurationMissing
        }
        return accessGroup
    }
    ...
    private static func save(
        ...
        guard status == errSecSuccess else {
            throw JLFBotSharedKeychainError.security(status)
        }
    }
    ...
    private static func readToken(
        ...
        guard let data = item as? Data,
            ...
            throw JLFBotSharedKeychainError.security(errSecDecode)
        }
        return token
    }
...
enum JLFBotSharedKeychainError: LocalizedError {
