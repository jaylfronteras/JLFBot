...
final class ConnectionTests: XCTestCase {
    ...
    func testParsesADesktopPairingInvite() throws {
        let token = "jlf_pair_" + String(repeating: "a", count: 43)
        let secretKey = "BIPBQ12_dWnF1DZLsTZO3Vg0NGjds5-jp9h3jhjr2To7bJelczS0LM82rfXV68PmSJhz2ePosj3fL974XckCpDU"
        let url = try XCTUnwrap(URL(string: "jlfbot://pair?address=macbook.tail1234.ts.net%3A8810&token=\(token)&code=004209&name=Milind%27s%20Mac&secretKey=\(secretKey)"))
    ...
    func testRejectsAPresentButInvalidSecureEntryKey() throws {
        ...
        let invalid = try XCTUnwrap(URL(string:
            "jlfbot://pair?address=mac.local&token=\(token)&secretKey=not-a-p256-key"))
        XCTAssertNil(PairingInvite.parse(invalid))
    }

    func testPairingConsentShowsNormalizedOriginInsteadOfTrustingQRName() throws {
        let url = try XCTUnwrap(URL(string:
            "jlfbot://pair?address=https%3A%2F%2FOTHER.Example%3A9443%2F" +
    ...
    func testParsesAnOlderCodeOnlyPairingInvite() throws {
        let url = try XCTUnwrap(URL(string: "jlfbot://pair?address=mac.local&code=004209"))
        let invite = try XCTUnwrap(PairingInvite.parse(url))
    ...
    func testLegacyTailnetInviteDropsUnselectedLocalFallbackKinds() throws {
        let url = try XCTUnwrap(URL(string:
            "jlfbot://pair?address=macbook.tail1234.ts.net%3A8810&code=004209" +
            "&hosts=macbook.tail1234.ts.net,192.168.1.42,jlfbot-aa.local"))
        let invite = try XCTUnwrap(PairingInvite.parse(url))
    ...
    func testHostedTypedInviteCannotRetainDirectFallbacks() throws {
        ...
        let token = "jlf_pair_" + String(repeating: "a", count: 43)
        let url = try XCTUnwrap(URL(string:
            "jlfbot://pair?address=192.168.1.42%3A8810&token=\(token)&endpoints=\(encoded)"))
    ...
    func testRejectsMalformedOrDowngradedTypedEndpoints() throws {
        ...
        let invalidBase64 = try XCTUnwrap(URL(string:
            "jlfbot://pair?address=192.168.1.42%3A8810&token=\(token)&endpoints=not-json"))
    ...
    func testSanitizesFallbackHostsAndKeepsOnlyTheConfirmedLocalOrigin() throws {
        ...
        let url = try XCTUnwrap(URL(string:
            "jlfbot://pair?address=mac.local&code=004209&hosts=%20mac.local%20,other.local,,bad%2Fslash,has%20space"))
        let invite = try XCTUnwrap(PairingInvite.parse(url))
        XCTAssertEqual(invite.connection.hosts, ["mac.local"])

        // and an invite with no usable candidate keeps the single address
        let empty = try XCTUnwrap(URL(string: "jlfbot://pair?address=mac.local&code=004209&hosts=bad%2Fslash"))
    ...
    func testAPairResponseWithAndWithoutHostsDecodes() throws {
        let older = Data(#"{"token":"jlf_x","device":{"id":"d","name":"p","createdAt":1,"lastSeenAt":1},"serverName":"Mac"}"#.utf8)
        XCTAssertNil(try JSONDecoder().decode(PairResponse.self, from: older).hosts)

        let newer = Data(#"{"token":"jlf_x","device":{"id":"d","name":"p","createdAt":1,"lastSeenAt":1},"serverName":"Mac","hosts":["a.ts.net","192.168.1.42"]}"#.utf8)
        XCTAssertEqual(try JSONDecoder().decode(PairResponse.self, from: newer).hosts, ["a.ts.net", "192.168.1.42"])

        let typed = Data(#"{"token":"jlf_x","device":{"id":"d","name":"p","createdAt":1,"lastSeenAt":1},"serverName":"Mac","endpoints":[{"url":"https://mac.example","kind":"hosted","priority":0}]}"#.utf8)
    ...
    func testRejectsAnUntrustedOrMalformedPairingInvite() throws {
        ...
        XCTAssertNil(PairingInvite.parse(try XCTUnwrap(URL(string: "jlfbot://pair?address=mac.local&code=12345"))))
        XCTAssertNil(PairingInvite.parse(try XCTUnwrap(URL(string: "jlfbot://pair?address=mac.local&token=weak"))))
        XCTAssertNil(PairingInvite.parse(try XCTUnwrap(URL(string: "jlfbot://pair?address=mac.local&token=weak&code=123456"))))
        XCTAssertNil(PairingInvite.parse(try XCTUnwrap(URL(string: "jlfbot://pair?address=host%2Fpath&code=123456"))))
        XCTAssertNil(PairingInvite.parse(try XCTUnwrap(URL(string: "jlfbot://pair?address=one.local&address=two.local&code=123456"))))
    }
