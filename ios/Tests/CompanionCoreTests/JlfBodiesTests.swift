...
///
/// The parser used to live in `ios/App/JlfAvatar.swift`, where `swift test`
...
final class JlfBodiesTests: XCTestCase {
    func testEveryBodyParsesToAClosedPathInsideTheFaceBox() {
        for id in JlfBodies.order {
            ...
            XCTAssertLessThanOrEqual(
                bounds.maxX, JlfSilhouette.faceBox + 1, "\(id) overflows the face box to the right")
            XCTAssertLessThanOrEqual(
                bounds.maxY, JlfSilhouette.faceBox + 1, "\(id) overflows the face box below")
        }
    }

    func testUnknownBodyFallsBackToTheCursor() {
        XCTAssertEqual(JlfBodies.body("hexagram").id, "cursor")
        XCTAssertEqual(JlfBodies.body(nil).id, "cursor")
        XCTAssertEqual(JlfBodies.body("").id, "cursor")
    }
    ...
    func testAnUnknownBodyDrawsTheCursor() {
        XCTAssertEqual(
            JlfSilhouette.faceBoxBounds("hexagram"), JlfSilhouette.faceBoxBounds("cursor"))
        XCTAssertEqual(JlfSilhouette.faceBoxBounds(nil), JlfSilhouette.faceBoxBounds("cursor"))

        let unknown = JlfSilhouette.anchor("hexagram")
        let cursor = JlfSilhouette.anchor("cursor")
    ...
    func testEveryAnchorPlacesAFaceInsideItsBody() {
        for id in JlfBodies.order {
            let anchor = JlfSilhouette.anchor(id)
    ...
    func testTheParserUnderstandsTheCatalogsPathGrammar() {
        ...
        let square = JlfSilhouette.parse(
            """
        ...
        let negative = JlfSilhouette.parse("M-2.5 0 C-2.5 0 1e1 -5 10 -5Z")
    ...
    func testEachBodyIsParsedOnceAndCachedSeparately() {
        for id in JlfBodies.order {
            XCTAssertTrue(
                JlfSilhouette.inFaceBox(id) === JlfSilhouette.inFaceBox(id),
                "\(id) was parsed twice")
        }
        XCTAssertFalse(JlfSilhouette.inFaceBox("cursor") === JlfSilhouette.inFaceBox("star"))
    }
}
