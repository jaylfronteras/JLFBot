...
The server receives `JLFBOT_RESOURCES_PATH` from Electron. It resolves an explicit
`JLFBOT_AGENT_BROWSER_PATH` override first, then the complete bundled engine and
...
There is no recording control in JLFBOT's browser panel.

## Linux sandbox

Use the `.deb` on Ubuntu 24.04. Its package hooks install a narrowly scoped
AppArmor policy for the root-owned browser executable under `/opt/JLFBot`.
...
On macOS, use `/absolute/JLFBot.app/Contents/Resources`. On Linux, run as
an unprivileged user against the installed `.deb` at
`/opt/JLFBot/resources`. This check creates its own empty home and local
...
Windows uses an explicitly identified JLFBot build of agent-browser
