...
folder on the computer running JLFBot — nothing is stored anywhere else,
...
disk, those buttons only work from the computer running JLFBot — a
...
`~/.jlfbot/memory-journal/<botId>.ndjson`. It lives *outside* the
