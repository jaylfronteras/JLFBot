...
Two zero-code ways to run JLFBot bots on an engine the app doesn't ship.
Both live in `~/.jlfbot/config.json` under `"instances"`; restart the app
...
- **Sign in first.** The driver has no auth flow of its own — run the CLI once
  in a terminal and log in there; JLFBot spawns it with your login intact.
