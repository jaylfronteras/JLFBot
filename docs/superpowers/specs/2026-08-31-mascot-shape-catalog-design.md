...
> `scripts/mascot-bodies/`, `JlfBodies.swift`. The repo enforces
...
built inline in `Avatar.tsx`. On the phone, `JlfSilhouette` hardcodes the same
outline as a 4KB string with hand-tuned transforms, and `JlfFaceData.anchor` is
...
`JlfAvatar.swift`'s own header predicts: "starts close and drifts every time
...
- `shared/mascot-shapes.ts` — the TypeScript catalog, the id union, and the zod
  schema.
- `ios/Sources/CompanionCore/JlfBodies.swift` — the Swift catalog, keyed by the same ids.

Both carry a "generated, do not hand-edit" header, matching the existing
convention in `JlfFaceData.swift`.

Why absolute cubics: the iOS path parser in `JlfSilhouette.parse()` understands
...
Of the 29 `JlfAvatar` call sites, only those rendering a specific bot change.
...
`JlfShapes.swift` holds the ten outlines and their anchors. `JlfSilhouette`
becomes a lookup over that catalog rather than a single static path;
`JlfFaceData.anchor` likewise.
...
`JlfAvatar` gains `shape: String = "cursor"`. `BotAvatarView` passes
