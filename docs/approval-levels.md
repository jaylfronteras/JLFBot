...
provider's own permission modes, passed through. JLFBot does not judge an
...
Access, JLFBOT also applies its own configuration tools without another approval.
...
their existing JLFBOT configuration review cards. Full actions return an applied
...
ends. JLFBot keeps no standing grant for a provider's tool. It is not
...
Native modes require a CLI version that supports them; JLFBot does not
silently substitute unrestricted access when a mode is rejected.

### Read-only integration tools

JLFBot's built-in agents MCP server describes nine scoped reads with
...
provider's session. Its quieter default is Full access; JLFBot keeps its
