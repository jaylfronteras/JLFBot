# JLFBot harness implementation plan
...
(pi, agent-orchestrator, deepseek-harness) and asking what JLFBot lacked by
...
**The reframe that reorders everything.** JLFBot is itself a harness. The
...
**On-disk forward compatibility.** `~/.jlfbot` is shared across app versions.
