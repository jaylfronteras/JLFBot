...
JLFBOT uses Qwen's native ACP model selector and requires the CLI to confirm the
...
`session/load` of its own live session, JLFBOT closes the child and pays the
...
on completion. This proves JLFBOT's integration contract, not real provider auth
