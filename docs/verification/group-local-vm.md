...
tracked separately in [#860](https://github.com/jaylfronteras/JLFBot/issues/860).
...
`JLFBOT_VERIFY_PODMAN` to the absolute executable path and `JLFBOT_VERIFY_MACHINE` to that connection,
then run:
...
`.jlfbot-scratch/verification-logs/group-vm-routing.json` only after cleanup succeeds.
