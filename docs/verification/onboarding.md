...
```sh
node --experimental-strip-types scripts/verify-onboarding-ui.ts /tmp/jlfbot-verify-data-XXXXXX/ui.json
```
...
are retained in `.jlfbot-scratch/verify-evidence/onboarding/` with a PASS line for
...
| Packaged desktop with the organisation bridge | An optional "Using JLFBot at work?" row on the engines beat; a signed-in Company engine counts as ready | `src/components/onboarding/beats/OrganisationRow.test.ts`, `src/components/onboarding/beats/EnginesBeat.test.ts` (fake bridge) |
...
default Admin. Screenshots are in `.jlfbot-scratch/verify-evidence/onboarding/`.
