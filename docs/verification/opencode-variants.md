...
The optional functional test launches the real JLFBot server, its OpenCode
...
```sh
JLFBOT_OPENCODE_E2E_CLI=/absolute/path/to/opencode pnpm exec vitest run server/opencode-variants.e2e.test.ts
```

Without `JLFBOT_OPENCODE_E2E_CLI`, Vitest reports this test as skipped. A skip does
...
The checks require JLFBot to retain this catalog metadata, advertise its
...
JLFBot server with the same disposable data directory.
...
This proves API persistence and actual JLFBOT → ACP → OpenCode → SDK transport
against a simulated provider. It does not establish acceptance by any real
provider, an upstream gateway's routing, or the JLFBot GUI. The fixture
