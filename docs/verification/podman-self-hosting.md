...
umask 077
cat > .env.fixture <<EOF
COMPOSE_PROJECT_NAME=jlfbot-podman-verify
JLFBOT_IMAGE_TAG=verify
JLFBOT_DATA_ROOT=$fixture
PODMAN_SOCKET=/run/user/$(id -u)/podman/podman.sock
JLFBOT_PORT=28799
JLFBOT_WEBHOOK_PORT=28800
JLFBOT_HTTP_PORT=28880
JLFBOT_PUBLIC_URL=http://localhost:28880
JLFBOT_HTTPS_HOST=https-disabled.invalid
ENGINES=
EOF
...
Use the candidate checkout's `control-jlfbot.ts` on the host. The explicit fixture
...
2. Use `control-jlfbot.ts new-bot --name "Fixture A" --url URL` and repeat for B.
