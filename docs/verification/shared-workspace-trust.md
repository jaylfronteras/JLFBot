...
- **Loopback trust.** At start-up the server logs `local requests: owner
  ...
  `JLFBOT_ADMIN_*` setting; shared-workspace Full access only works there)
  defaults to `service`; a self-hosted server keeps `owner`;
  `JLFBOT_LOOPBACK_TRUST=owner|service` overrides (anything else means
...
- **The CLI under service trust.** `jlfbot serve` hands the server it
  ...
  server running. Later `jlfbot pair` and `jlfbot sessions` are
...
- **Decision log retention.** Rows go to `<data>/decisions/YYYY-MM.ndjson`. A
  ...
  default; `decisions.retentionDays` or `JLFBOT_DECISION_RETENTION_DAYS`). An
...
- **Settings on a portal-membership workspace.** `GET /api/config` reports
  ...
  `<JLFBOT_ADMIN_URL>/people?workspace=<slug>`); Remote access offers no pairing
...
- `server/card-answerers.e2e.test.ts` boots a server with one admin and two
  ...
  anyone's card, and under `JLFBOT_LOOPBACK_TRUST=service` a session-less caller
...
- `server/cli-service-trust.e2e.test.ts` runs the real `jlfbot serve`
  with `JLFBOT_LOOPBACK_TRUST=service`: it prints a pairing code and keeps
...
On a disposable worktree rebased onto JLFBot main `0bb37982`:
