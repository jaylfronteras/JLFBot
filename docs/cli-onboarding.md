...
If the workspace starts but its HTTPS check fails, the local workspace remains usable and no phone code is created. Fix the connection and run `jlfbot pair` in another terminal to try again. A missing account, connector, or Tailscale prerequisite can prevent startup; follow the printed error, or use `jlfbot --local`. To add phone access after skipping it, stop the server, run `jlfbot setup`, then start it again.
...
| Command | Use |
| --- | --- |
| `jlfbot` | Set up once, then start with saved settings. |
| `jlfbot setup` | Revisit AI and optional phone setup, save, and exit without starting. This is not a reset. |
| `jlfbot --no-open` | Start without opening a browser. |
| `jlfbot --local` | Ignore saved remote access for this launch; keep the saved choice for next time. |
| `jlfbot --no-pair` | Suppress phone setup prompts and pairing invitations. This does **not** turn off saved remote access; use `--local` for that. |
...
```sh
jlfbot setup --data-dir /path/to/jlfbot-data --port 8799
jlfbot --data-dir /path/to/jlfbot-data --port 8799
```
...
Ctrl-C during AI setup leaves unsaved JLFBOT changes unapplied. Installations and provider sign-ins already completed remain available. Ctrl-C during the later phone step keeps the AI setup you already saved, exits without starting a server, and does not undo an account sign-in already completed. Run `jlfbot setup` to continue; no destructive reset is needed.
