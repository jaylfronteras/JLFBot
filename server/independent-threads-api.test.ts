...
describe("independent bot tasks through the isolated control surface", () => {
  ...
  it("queues coordinated work behind a peer's approval even with a spare thread, and delivers it once without another user prompt", async () => {
    ...
    const token = (await dump(models[0])).mcpConfig.mcpServers.agents.env.JLFBOT_COMMS_TOKEN;
  ...
  it("rejects blank memory replacements, caps new titles, and retains project files after deletion", async () => {
    ...
    const token = launched.mcpConfig.mcpServers.agents.env.JLFBOT_COMMS_TOKEN;
  ...
  it("keeps A and B independent across selection, models, approvals, and stopping A", async () => {
    ...
    expect(launchedA.env.JLFBOT_FIXTURE_CWD).toBe(realpathSync(join(session.info.dataDir, "task-workspaces", botId, taskA)));
    expect(launchedB.env.JLFBOT_FIXTURE_CWD).toBe(realpathSync(join(session.info.dataDir, "task-workspaces", botId, taskB)));

    const tokenA = launchedA.mcpConfig.mcpServers.agents.env.JLFBOT_COMMS_TOKEN;
    const tokenB = launchedB.mcpConfig.mcpServers.agents.env.JLFBOT_COMMS_TOKEN;
  ...
  it("cancels a detached routine on its captured provider after an idle sibling changes the default", async () => {
    ...
    const routine = await api("POST", "/api/routines", {
      name: "Captured routine provider", prompt: "ROUTINE_PROVIDER_OWNER", botId, runOn: "jlf", enabled: false,
  ...
  it("refuses a second engine in the same selected project folder until its owner stops", async () => {
    ...
    expect((await dump(models[0])).env.JLFBOT_FIXTURE_CWD).toBe(realpathSync(cwd));
    ...
    expect((await dump(models[1])).env.JLFBOT_FIXTURE_CWD).toBe(realpathSync(cwd));
  ...
  it("runs an unattended task in the bot's own level, still carding what the provider asks, while a sibling runs attended", async () => {
    ...
    const hook = await api("POST", "/api/webhooks", { name: "Fixture event", prompt: "UNATTENDED_ONLY", botId, runOn: "jlf" });
