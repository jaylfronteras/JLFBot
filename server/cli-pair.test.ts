...
beforeEach(() => {
  ...
  fetchMock.mockImplementation(async (input, init) => {
    ...
    if (url === `${local}/.well-known/jlfbot/environment`) return Response.json({ environmentId: workspaceId });
    if ([advertisedOrigin, explicitOrigin].some((origin) => url === `${origin}/.well-known/jlfbot/environment`)) {
...
function expectPhonePairing(origin: string): void {
  ...
  expect(String(probes[0]![0])).toBe(`${origin}/.well-known/jlfbot/environment`);
