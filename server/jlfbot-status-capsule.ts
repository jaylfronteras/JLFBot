...
// External producer contract (aos-session-bridge): keep the upstream schema id and path.
...
export const JLFBOT_STATUS_CACHE_PATH = join(
  homedir(),
  ".local/state/aos-session-bridge/openmausbot/latest.json",
);

type JLFBotSlot = z.output<typeof slotSchema>;
type JLFBotUi = z.output<typeof uiSchema>;
type JLFBotCapsule = z.output<typeof capsuleSchema>;

export interface JLFBotStatusDigest {
...
export interface JLFBotStatusReadOptions {
  cachePath?: string;
  now?: Date;
}
...
function expectedUi(twoUp: boolean): JLFBotUi {
  return {
    two_up: twoUp,
...
function sameUi(left: JLFBotUi, right: JLFBotUi): boolean {
  return (
...
function unsignedDocument(capsule: JLFBotCapsule): JsonObject {
...
function validSlotState(slot: JLFBotSlot): boolean {
  return (
...
function validateCapsule(value: JsonValue): JLFBotCapsule | null {
...
function unknownDigest(
  reason: NonNullable<JLFBotStatusDigest["reason"]>,
  freshness: JLFBotStatusDigest["freshness"] = "unknown",
  receipt?: Pick<JLFBotStatusDigest, "observedAt" | "receiptSha256">,
): JLFBotStatusDigest {
  return {
    schema: SCHEMA,
    freshness,
...
export function readJLFBotStatus(
  options: JLFBotStatusReadOptions = {},
): JLFBotStatusDigest {
  const raw = privateCache(options.cachePath ?? JLFBOT_STATUS_CACHE_PATH);
  if (raw === null) return unknownDigest("missing_or_insecure");
  let capsule: JLFBotCapsule | null;
  try {
  ...
  const result: JLFBotStatusDigest = {
    schema: SCHEMA,
    freshness: "fresh",
    ...receipt,
...
export function jlfBotStatusSystemPrompt(options: JLFBotStatusReadOptions = {}): string {
  const status = readJLFBotStatus(options);
  const receipt = [
  ...
  const slots = status.slots.length
    ...
    : "none";
  return [
    "TRUSTED JLFBOT STATUS (read-only, validated, no transcript or credential data):",
