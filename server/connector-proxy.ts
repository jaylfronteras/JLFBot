...
// into first-class JLFBot chat cards. The agent never authors an auth
...
const UPSTREAM = process.env.JLFBOT_CONNECTOR_UPSTREAM_URL ?? "";
const HARNESS = process.env.JLFBOT_HARNESS_URL ?? "http://127.0.0.1:8799";
const BOT_ID = process.env.JLFBOT_BOT_ID ?? "";
const THREAD_ID = process.env.JLFBOT_THREAD_ID ?? "";
const TOKEN = process.env.JLFBOT_CONNECTOR_TOKEN ?? process.env.JLFBOT_COMMS_TOKEN ?? "";
...
function parsedHeaders(): Record<string, string> {
  try {
    const value: unknown = JSON.parse(process.env.JLFBOT_CONNECTOR_UPSTREAM_HEADERS ?? "{}");
...
function initializeResult(id: unknown, protocolVersion: unknown): Json {
  return {
    ...
    result: {
      ...
      capabilities: { tools: {} },
      serverInfo: { name: "jlfbot-connectors", version: "1" },
    },
  };
}
...
async function handle(message: Json): Promise<void> {
  ...
  if (method === "tools/call") {
    ...
    if (requests.length) {
      ...
      send(textResult(
        id,
        `JLFBot showed the user a secure connection card for ${labels}. End this turn now. The app will continue the task automatically after the connection finishes.`,
      ));
      return;
    }
    if (/WAIT_FOR_CONNECTIONS$/i.test(name)) {
      send(textResult(id, "JLFBot is handling connection completion and will continue the task automatically."));
      return;
    }
  }
  try {
