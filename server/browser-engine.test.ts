...
describe("deleting one browser session's saved logins", () => {
  function fixture(closeCode = 0) {
    const home = mkdtempSync(join(tmpdir(), "jlfbot-browser-cleanup-"));
    scratch.push(home);
  ...
  it("cannot launch a browser from close through inherited launch flags or native user config", async () => {
    ...
    expect(env?.AGENT_BROWSER_CONFIG).toBe(join(home, ".agent-browser", "jlfbot-managed-config.json"));
...
describe("restoring exactly one browser profile", () => {
  function fixture() {
    const home = mkdtempSync(join(tmpdir(), "jlfbot-browser-restore-"));
    scratch.push(home);
    const directory = join(home, ".agent-browser", "sessions");
  ...
  it("uses fixed-length restore identities even for overlapping legacy names", () => {
    ...
    expect(keys.every((key) => /^jlfbot-[a-f0-9]{64}$/.test(key))).toBe(true);
  ...
  it("does not overwrite an unexpected managed config or start a browser with it", async () => {
    const { directory, options } = fixture();
    const path = join(directory, "..", "jlfbot-managed-config.json");
...
describe("finding the browser engine", () => {
  ...
  it("does not reuse a pre-fix Windows managed install for a revised release", () => {
    const dataDir = join(tmpdir(), "jlfbot-versioned-browser-fixture");
    ...
    expect(resolveAgentBrowserBinary({ ...options, env: { PATH: "", JLFBOT_AGENT_BROWSER_PATH: old } })).toBe(old);
  });
  ...
  it.each(SUPPORTED_BROWSER_TARGETS)("uses the complete %s desktop bundle before old downloaded engines", (target) => {
    ...
    const env = { JLFBOT_RESOURCES_PATH: join(tmpdir(), "JLFBOT resources"), PATH: "" };
    const bundle = browserBundlePaths(join(env.JLFBOT_RESOURCES_PATH, "browser-engine"), target);
    ...
    files.add(external);
    expect(resolveAgentBrowserBinary({ ...options, env: { ...env, JLFBOT_AGENT_BROWSER_PATH: external } })).toBe(external);
  });

  it("mounts the bundled browser with no download and keeps explicit Chrome overrides", async () => {
    const resources = mkdtempSync(join(tmpdir(), "jlfbot-browser-resources-"));
    scratch.push(resources);
    const host = { platform: "linux" as const, arch: "x64" };
    const env = { JLFBOT_RESOURCES_PATH: resources, PATH: "" };
    ...
    expect(spec.env).not.toHaveProperty("JLFBOT_RESOURCES_PATH");
  ...
  it("prefers the explicit path, then the pinned download, then PATH, and reports why when nothing is there", () => {
    const dataDir = mkdtempSync(join(tmpdir(), "jlfbot-engine-"));
    ...
    files.add(override);
    expect(resolveAgentBrowserBinary({ dataDir, env: { ...env, JLFBOT_AGENT_BROWSER_PATH: override }, exists })).toBe(override);
    // an override that does not exist is an error, not a silent fallback
    expect(resolveAgentBrowserBinary({ dataDir, env: { ...env, JLFBOT_AGENT_BROWSER_PATH: join(dataDir, "missing", name) }, exists })).toBeNull();
...
describe("installing the browser engine", () => {
  it("downloads the pinned asset, verifies size and digest, and only then names the file", async () => {
    const dataDir = mkdtempSync(join(tmpdir(), "jlfbot-engine-install-"));
    scratch.push(dataDir);
  ...
  it("refuses a download whose bytes do not match the pin, and leaves nothing behind", async () => {
    const dataDir = mkdtempSync(join(tmpdir(), "jlfbot-engine-bad-"));
    scratch.push(dataDir);
...
describe("what a bot gets", () => {
  it("forwards the explicit Chrome path without copying arbitrary environment or overriding session isolation", () => {
    ...
    expect(spec.env).toEqual({
      ...
      AGENT_BROWSER_CONFIG: expect.stringContaining("jlfbot-managed-config.json"),
  ...
  it("makes one encryption key per data dir, private, and reuses it", () => {
    const dataDir = mkdtempSync(join(tmpdir(), "jlfbot-engine-key-"));
    scratch.push(dataDir);
