...
it("boots and reports an interrupted routine back to its source room", async () => {
  ...
  try {
    ...
    writeFileSync(join(dataDir, "routines.json"), JSON.stringify({ version: 1, routines: [], runs: [{
      ...
      botId: bot.id, target: "bot", runOn: "jlf", sourceThreadId: channel.activeTaskId,
    ...
    expect(runs[0]).toMatchObject({ status: "failed", error: "JLFBot restarted while this routine was running" });
