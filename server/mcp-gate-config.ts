 ...
 * bot servers directly as before. `JLFBOT_MCP_RESULT_BUDGET=0` is the escape
...
export function resultBudget(env: NodeJS.ProcessEnv = process.env): number {
  const raw = env.JLFBOT_MCP_RESULT_BUDGET;
...
export function gateServer(input: {
  ...
  return {
    ...
    env: {
      ...input.nodeEnv,
      JLFBOT_GATE_NAME: name,
      JLFBOT_GATE_UPSTREAM: JSON.stringify({ command: spec.command, args: spec.args ?? [], env: spec.env ?? {} }),
      JLFBOT_GATE_SPILL_DIR: spillDir(input.threadId),
      JLFBOT_GATE_BUDGET: String(budget),
    },
  };
}
