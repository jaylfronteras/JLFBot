...
// accounts.jlfbot.example.com. The exchange happens server-side, so a browser
...
export const ADMIN_EMAILS_ENV = "JLFBOT_SIGNIN_EMAILS";
export const MEMBER_EMAILS_ENV = "JLFBOT_SIGNIN_MEMBER_EMAILS";
...
export function createEmailSignIn(options: {
  ...
  const controlPlane = (): ControlPlaneClient => {
    ...
    if (!url) throw new Error("JLFBOT_CONTROL_PLANE_URL is set but is not an https address");
