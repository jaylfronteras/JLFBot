...
describe.skipIf(process.platform === "win32")("fleet agent over its socket", () => {
  ...
  beforeEach(() => {
    root = mkdtempSync(join(tmpdir(), "jlfbot-fleet-agent-"));
  ...
  async function boot(m: ReturnType<typeof machine>) {
    const logs: string[] = [];
    const server = await startFleetAgent({ socketPath, node: "/usr/bin/node", script: "/usr/lib/node_modules/jlfbot/cli.js", root: m.root, deps: m.deps, auditFile: join(m.root, "audit.jsonl"), licenseKey: "omb1.k", now: () => new Date("2026-09-10T12:00:00Z") }, { log: (line) => logs.push(line) });
  ...
  it("creates, lists with this month's usage, edits users, suspends, deletes, and audits every action", async () => {
    ...
    m.files.set(layout.registryFile, JSON.stringify({ ...emptyRegistry("agentada.cc"), operator: "jlf" }));
    ...
    expect(listed).toMatchObject({ status: 200, body: { domain: "agentada.cc", operator: "jlf", workspaces: [{ slug: "acme", host: "acme.agentada.cc", port: 8810, live: "active", usage: { month: "2026-09", turns: 1, costUsd: 0.25 } }] } });
    expect(m.calls).toContain(`usage ${dataDir} as jlfbot-acme`);
    ...
    expect(m.calls).toContain("systemctl disable --now jlfbot@acme.service");
    ...
    expect(m.files.get(layout.fenceFile)).toContain("jlfbot-acme");
  ...
  it("lists service status without reading tenant usage or mutating the fleet", async () => {
    ...
    expect(m.calls).toEqual(["systemctl is-active jlfbot@alpha.service", "systemctl is-active jlfbot@beta.service"]);
  ...
  it("refuses bad names and unknown operations, and reports a failed step without the tool's secrets", async () => {
    ...
    expect(m.calls).not.toContain("systemctl is-active jlfbot@beta.service");
  });
  ...
  it("serializes complete mutations and exposes the reservation while creation is pending", async () => {
    ...
    const m = machine(root, { beforeRun: async (argv) => {
      if (argv[0] === "useradd" && argv.at(-1) === "jlfbot-alpha") await blocked;
    } });
    const layout = fleetLayout(root);
    ...
    try {
      await vi.waitFor(() => expect(m.calls.some((call) => call.endsWith("jlfbot-alpha"))).toBe(true));
      ...
      expect(m.calls.some((call) => call.endsWith("jlfbot-beta"))).toBe(false);
      release();
      ...
      expect(m.files.get(layout.fenceFile)).toContain("jlfbot-alpha");
      expect(m.files.get(layout.fenceFile)).toContain("jlfbot-beta");
    } finally {
      release();
      await alpha;
    }
  ...
  it("reports unsafe usage as unavailable without failing the list or exposing ledger content", async () => {
    ...
    m.deps.usage = (dataDir, owner, now) => {
      expect(owner).toBe(dataDir.includes("/alpha/") ? "jlfbot-alpha" : "jlfbot-beta");
      expect(now.toISOString()).toBe("2026-09-10T12:00:00.000Z");
      if (owner === "jlfbot-alpha") throw new Error("unsafe fixture-secret ledger");
