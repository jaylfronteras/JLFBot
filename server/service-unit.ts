// `jlfbot service install`: keep the server running across reboots.
// Renders a systemd unit (Linux) or a launchd agent (macOS) that runs the
// same `jlfbot serve …` the operator just used, and either installs it
...
export const SYSTEMD_UNIT_NAME = "jlfbot.service";
export const LAUNCHD_LABEL = "com.jlfbot.serve";
...
export function systemdUnit(spec: ServiceSpec): string {
  const lines = [
    "# Written by `jlfbot service install`. Re-run it to change the options.",
    "[Unit]",
    `Description=JLFBot${spec.label ? ` (${spec.label})` : ""}`,
    ...
    `Environment=JLFBOT_DATA_DIR=${spec.dataDir}`,
...
export function unstableInstallWarning(script: string): string | null {
  ...
  if (/\/_npx\//.test(normalized) || /\/\.npm\/_npx\//.test(normalized)) {
    return `this command runs from an npx cache (${dirname(script)}), which npm may delete at any time. Install it permanently first (npm install -g jlfbot) and run \`jlfbot service install\` from that install.`;
  }
  return null;
}
...
export function currentUser(): string {
  try {
    ...
    return process.env.USER || process.env.USERNAME || "jlfbot";
  }
}
