...
export const VPS_MANAGED_LABEL = "com.jlfbot.vps";
export const VPS_CONTAINER_LABEL = "com.jlfbot.container";
export const VPS_ENVIRONMENT_LABEL = "com.jlfbot.environment";
export const VPS_VIEWER_LABEL = "com.jlfbot.vps-viewer";
export const VPS_CONTAINER_PREFIX = "jlfbot-vps";
...
const MANAGED_VPS_CONTAINER_NAME = /^jlfbot-vps-[a-z0-9]{1,12}-[a-f0-9]{12}$/;
const IMAGE_ID = /^sha256:[a-f0-9]{64}$/i;
const PIDS_LIMIT = 512;
const SCREENSHOT_PATH = "/tmp/jlfbot-vps-preview.png";
...
function statusProblem(status: VpsComputerStatus): string | null {
  ...
  if (!status.image) return `Prepare the pinned JLFBot Cua image on the VPS (Driver ${CUA_DRIVER_VERSION})`;
  if (status.container === "missing") return "No JLFBot container exists for this bot on the VPS";
  if (!status.imageMatches) return "The VPS container uses an incompatible or untrusted JLFBot image";
  if (!status.managed) return "The VPS container name is occupied by a container JLFBot did not create";
  ...
  if (status.security === "unsafe") return "The VPS container is missing JLFBot safety limits";
  if (status.container === "stopped") return "The JLFBot VPS container is stopped";
  ...
  const operation = async () => {
    ...
    try {
      ...
      if (action === "provision") {
        ...
        // never docker-rm a container JLFBot did not create, even one
        // squatting on our name.
        if (before.container === "missing") return before;
        if (!before.managed) {
          throw Object.assign(
            new Error("The VPS container name is occupied by a container JLFBot did not create — remove it on the VPS yourself"),
            { status: 409 },
          );
        }
