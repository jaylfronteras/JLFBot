...
function fakeLayer(registerSource: string, file = "index.ts"): string {
  const dir = mkdtempSync(join(tmpdir(), "jlfbot-enterprise-"));
...
afterEach(async () => {
  await loadEnterpriseLayer({ dir: join(tmpdir(), "jlfbot-enterprise-absent"), licenseKey: undefined });
...
describe("enterprise hook point", () => {
  it("requires an explicit operator policy on a complete portal-managed non-desktop workspace", () => {
    const env = { JLFBOT_ADMIN_URL: "https://admin.example.test", JLFBOT_ADMIN_WORKSPACE: "acme", JLFBOT_PUBLIC_URL: "https://acme.example.test",
      JLFBOT_ADMIN_MEMBERSHIP: "portal", JLFBOT_SHARED_WORKSPACE_FULL_ACCESS: "1" };
    expect(sharedWorkspaceFullAccessConfigured(env)).toBe(true);
    for (const patch of [
      { JLFBOT_SHARED_WORKSPACE_FULL_ACCESS: undefined }, { JLFBOT_SHARED_WORKSPACE_FULL_ACCESS: "true" },
      { JLFBOT_DESKTOP_PARENT: "1" }, { JLFBOT_ADMIN_MEMBERSHIP: "local" }, { JLFBOT_ADMIN_MEMBERSHIP: undefined },
      { JLFBOT_ADMIN_URL: "http://admin.example.test" }, { JLFBOT_PUBLIC_URL: "" }, { JLFBOT_ADMIN_WORKSPACE: "" },
    ]) expect(sharedWorkspaceFullAccessConfigured({ ...env, ...patch })).toBe(false);
    expect(sharedWorkspaceFullAccessConfigured({ JLFBOT_SHARED_WORKSPACE_FULL_ACCESS: "1" })).toBe(false);
  });
  it("does not create a hosted bridge unless opted in, and marks partial configuration as hosted", () => {
    ...
    expect(hostedWorkspaceConfigured({ JLFBOT_PUBLIC_URL: "https://legacy.example.test" })).toBe(false);
    expect(hostedWorkspaceConfigured({ JLFBOT_ADMIN_URL: "" })).toBe(true);
    expect(hostedWorkspaceConfigured({ JLFBOT_ADMIN_WORKSPACE: "acme" })).toBe(true);
    expect(hostedWorkspaceConfigured({ JLFBOT_ADMIN_MEMBERSHIP: "portal" })).toBe(true);
    ...
    expect(createWorkspaceAccess({ sessions, cookieName: "session", closeSessionStreams() {}, env: { JLFBOT_ADMIN_WORKSPACE: "acme" } })).toBeNull();
  });
  it("requires complete valid hosted configuration before opting into portal membership", () => {
    ...
    expect(hostedWorkspaceConfiguration({ ...env, JLFBOT_ADMIN_MEMBERSHIP: "portal" })?.portalMembership).toBe(true);
    expect(hostedWorkspaceConfiguration({ ...env, JLFBOT_ADMIN_MEMBERSHIP: "local" })?.portalMembership).toBe(false);
    for (const patch of [{ JLFBOT_ADMIN_URL: "" }, { JLFBOT_PUBLIC_URL: "http://acme.example.test" }, { JLFBOT_ADMIN_WORKSPACE: "" }, { JLFBOT_ADMIN_MEMBERSHIP: "PORTAL" }]) {
      expect(hostedWorkspaceConfiguration({ ...env, JLFBOT_ADMIN_MEMBERSHIP: "portal", ...patch })).toBeNull();
    }
    const dir = fakeLayer("export const fixture = true;");
    const invalid = { JLFBOT_ADMIN_MEMBERSHIP: "portal" };
  ...
  it("passes the live admin entitlement to the optional dynamically loaded bridge", async () => {
    ...
    const sessions = new SessionRegistry({ file: join(dir, "sessions.json") });
    const bridge = createWorkspaceAccess({ sessions, cookieName: "session", closeSessionStreams() {}, env: { JLFBOT_ADMIN_WORKSPACE: "acme" } });
  ...
  it("is the open-source edition when the folder is absent, and says so if a key was set anyway", async () => {
    const absent = join(tmpdir(), "jlfbot-enterprise-absent");
    ...
    expect(status.notice).toContain("JLFBOT_LICENSE_KEY is set but no enterprise layer exists");
    expect(entitled("whitelabel")).toBe(false);
    expect(describeEdition(status)).toContain("open-source edition (JLFBOT_LICENSE_KEY is set");
  });
  ...
  it("registers the layer's entitlements from the key", async () => {
    ...
    expect(describeEdition(status)).toBe("jlfbot enterprise edition for Acme until 2027-01-01: sso, whitelabel");
  });
  ...
  it("loads a compiled layer (server/index.js) the way an image ships it", async () => {
    ...
    expect(describeEdition(status)).toBe("jlfbot enterprise edition for Built: admin");
  });
  ...
  it("warns for 30 days, keeps working for a 7-day grace, then stops granting features without a restart", async () => {
    ...
    expect(licenseWarning(soon)).toBe("JLFBOT_LICENSE_KEY expires on 2027-01-01 (in 30 days); renew it before then to keep enterprise features");
    ...
    expect(grace.notice).toBe("JLFBOT_LICENSE_KEY expired on 2027-01-01; enterprise features keep working until 2027-01-08 while it is renewed");
  ...
  it("finds the layer beside the server root, then inside it (the Docker and desktop layout), unless JLFBOT_ENTERPRISE_DIR says where", async () => {
    const root = mkdtempSync(join(tmpdir(), "jlfbot-layout-"));
    dirs.push(root);
    ...
    expect(enterpriseLayerDirs({ JLFBOT_ENTERPRISE_DIR: "/opt/layer" }, serverRoot)).toEqual(["/opt/layer"]);
    // nothing anywhere: the notice names every place it looked
    const none = await loadEnterpriseLayer({ env: {}, serverRoot, licenseKey: "k" });
    expect(none.notice).toBe(`JLFBOT_LICENSE_KEY is set but no enterprise layer exists at ${join(root, "enterprise")} or ${join(serverRoot, "enterprise")}`);
    ...
    expect((await loadEnterpriseLayer({ env: { JLFBOT_ENTERPRISE_DIR: explicit }, serverRoot, licenseKey: "k" })).notice)
      .toBe(`JLFBOT_LICENSE_KEY is set but no enterprise layer exists at ${explicit}`);
    layer(explicit, "Explicit");
    expect(await loadEnterpriseLayer({ env: { JLFBOT_ENTERPRISE_DIR: explicit }, serverRoot, licenseKey: "k" })).toMatchObject({ customer: "Explicit" });
  });
  ...
  it("tells the operator when the layer is present but no key is configured", async () => {
    ...
    expect(status).toEqual({ edition: "oss", features: [], notice: "enterprise layer present but JLFBOT_LICENSE_KEY is not set" });
  });
