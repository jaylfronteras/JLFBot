...
const HELPER = join(dirname(fileURLToPath(import.meta.url)), "jlfbot-hook.ts");
const dir = mkdtempSync(join(tmpdir(), "jlfbot-hook-"));
...
describe("jlfbot-hook helper", () => {
  it.each(["context", "hookSpecificOutput"])("flushes large %s replies before exiting", async (field) => {
    ...
    const result = await run(JSON.stringify({ hook_event_name: "SessionStart" }), { JLFBOT_HOOK_URL: url, JLFBOT_HOOK_TOKEN_FILE: tokenFile });
  ...
  it("does not forward oversized hook input", async () => {
    ...
    const result = await run(JSON.stringify({ hook_event_name: "PostToolUse", tool_response: "x".repeat(1024 * 1024) }), { JLFBOT_HOOK_URL: url, JLFBOT_HOOK_TOKEN_FILE: tokenFile });
    expect(result.code).toBe(0);
  ...
  it("does not follow redirects with a private hook payload", async () => {
    ...
    const result = await run(JSON.stringify({ hook_event_name: "PostToolUse", tool_response: "private output" }), { JLFBOT_HOOK_URL: url, JLFBOT_HOOK_TOKEN_FILE: tokenFile });
    expect(result.code).toBe(0);
    expect(result.stdout).toBe("");
  ...
  it("exits 0 quietly on garbage input and no harness", async () => {
    const r = run("not json at all", { JLFBOT_HOOK_URL: "http://127.0.0.1:1", JLFBOT_HOOK_TOKEN_FILE: tokenFile });
  ...
  it("posts the event with the bearer from the token file and prints the harness's hookSpecificOutput", async () => {
    ...
    const result = await run(JSON.stringify(payload), { JLFBOT_HOOK_URL: url, JLFBOT_HOOK_TOKEN_FILE: tokenFile });
    expect(result.code).toBe(0);
  ...
  it("gives up within its budget when the harness never answers, still exiting 0", async () => {
    ...
    const result = await run(JSON.stringify({ hook_event_name: "Stop" }), { JLFBOT_HOOK_URL: url, JLFBOT_HOOK_TOKEN_FILE: tokenFile, JLFBOT_HOOK_TIMEOUT_MS: "800" });
    expect(result.code).toBe(0);
    expect(result.stdout).toBe("");
  ...
  it("prints nothing when the harness returns no hookSpecificOutput", async () => {
    ...
    const result = await run(JSON.stringify({ hook_event_name: "PostToolUse", tool_use_id: "x" }), { JLFBOT_HOOK_URL: url, JLFBOT_HOOK_TOKEN_FILE: tokenFile });
    expect(result.code).toBe(0);
    expect(result.stdout).toBe("");
  }, 15_000);
});

describe("jlfbot-hook helper: SessionStart context", () => {
  it("prints the harness's plain-text context on SessionStart, because Claude Code reads stdout as context there", async () => {
    ...
    const result = await run(JSON.stringify({ hook_event_name: "SessionStart", source: "compact", session_id: "s1" }), { JLFBOT_HOOK_URL: url, JLFBOT_HOOK_TOKEN_FILE: tokenFile });
    expect(result.code).toBe(0);
  ...
  it("ignores a context field on events where stdout is not context", async () => {
    ...
    const result = await run(JSON.stringify({ hook_event_name: "PostToolUse", tool_use_id: "t" }), { JLFBOT_HOOK_URL: url, JLFBOT_HOOK_TOKEN_FILE: tokenFile });
    expect(result.code).toBe(0);
    expect(result.stdout).toBe("");
  }, 15_000);
});
