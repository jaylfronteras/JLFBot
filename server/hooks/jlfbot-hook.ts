...
//   2. It has a hard budget (JLFBOT_HOOK_TIMEOUT_MS, default 4000) below Claude
...
//      from JLFBOT_HOOK_TOKEN_FILE at run time — the token rotates per turn
...
const requestedBudget = Number(process.env.JLFBOT_HOOK_TIMEOUT_MS);
const budgetMs = Number.isFinite(requestedBudget) && requestedBudget > 0 ? Math.min(requestedBudget, 4_000) : 4_000;
const MAX_INPUT_BYTES = 1024 * 1024;
const url = process.env.JLFBOT_HOOK_URL ?? "";
const tokenFile = process.env.JLFBOT_HOOK_TOKEN_FILE ?? "";
