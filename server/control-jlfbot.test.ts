...
import {
  ...
  runControlOmb,
} from "../scripts/control-jlfbot.ts";
import { installedChrome, UI_MUTATING } from "../scripts/testing/control-jlfbot-ui.ts";
import { removeTempDir } from "./testing/cleanup.ts";

describe("control-jlfbot command mapping", () => {
  ...
  it("keeps every ui verb off discovery: the launch handle is required, whatever the environment says", async () => {
    const env = { JLFBOT_URL: "http://127.0.0.1:19999", JLFBOT_PORT: "19999" };
  ...
  it("refuses a handle whose launch has already been stopped", async () => {
    const dir = mkdtempSync(join(tmpdir(), "jlfbot-ui-handle-"));
    try {
      const handle = join(dir, "ui.json");
      writeFileSync(handle, JSON.stringify({
        url: "http://127.0.0.1:19999", previewUrl: "http://127.0.0.1:5199/__threads.html", session: "jlfbot-ui-19999",
  ...
  it("finds the newest Chrome for Testing that agent-browser install unpacked", () => {
    const dir = mkdtempSync(join(tmpdir(), "jlfbot-ui-tools-"));
    try {
  ...
  it("runs directly under Node's strip-only TypeScript loader", () => {
    const result = spawnSync(process.execPath, [
      ...
      join(process.cwd(), "scripts", "control-jlfbot.ts"),
      "help",
    ], { encoding: "utf8" });
    expect(result.status).toBe(0);
    expect(result.stdout).toContain("control-jlfbot");
  });
  ...
  it("maps bounded reads and dry-run actions without reimplementing them", async () => {
    ...
    const env = { JLFBOT_URL: "http://127.0.0.1:19999" };
  ...
  it("forwards pinned task IDs for sends, reads, waits, interrupts, and model changes", async () => {
    const callTool = vi.fn(async (name: string, args: Record<string, unknown>) => ({ name, args }));
    const dependencies = { callTool: callTool as any, env: { JLFBOT_URL: "http://127.0.0.1:19999" } };
...
describe("control-jlfbot isolated verification loop", () => {
  it.each([
    "tcp://127.0.0.1:2375",
  ...
  it("launches, drives a real fake-engine turn, and removes only its test data", async () => {
    const parentEnv = {
      ...
      JLFBOT_SKILLS_DIR: "/must/not/reach/the/fixture",
    ...
    const env = { JLFBOT_URL: session.info.url };
    try {
      ...
      expect(fixtureEnv).not.toHaveProperty("JLFBOT_SKILLS_DIR");
  ...
  it("scripts the fake engine's tool calls from the launcher's environment", async () => {
    const session = await launchVerificationServer({
      ...process.env,
      FAKE_CLAUDE_TOOL_CALLS: '[{"name":"Bash","input":{"command":"pnpm control:jlfbot doctor","password":"fixture-secret"},"output":{"text":"fixture healthy","api_key":"fixture-output-secret"},"ok":true},{"name":"Bash","input":{"command":"false"},"output":"fixture command failed","ok":false}]',
    });
    const env = { JLFBOT_URL: session.info.url };
    try {
      ...
      expect(recorded[0].tool?.input).toContain("pnpm control:jlfbot doctor");
