...
const mocks = vi.hoisted(() => ({
  ...
  fleetAccess: vi.fn(),
  FLEET_CREDENTIAL_ENV: "JLFBOT_INSTALLATION_CREDENTIAL",
  denyLogOpen: false,
  spawn: vi.fn(),
...
beforeEach(() => {
  ...
  mocks.tunnelAccess.mockReturnValue({ endpoint: "https://fixture.jlfbot.test" });
...
describe("CLI startup lifecycle", () => {
  ...
  it("reopens a matching desktop workspace even when its server PID differs from the lease owner", async () => {
    ...
    writeFileSync(join(dataDir, "jlfbot-server.lease"), JSON.stringify({
  ...
  it.each(["mismatched", "unreachable"])("never mints a phone code for a %s public endpoint", async (failure) => {
    ...
    expect(requests).toContain("https://fixture.example.test/.well-known/jlfbot/environment");
  });
  ...
  it("offers Android client pairing only after verifying the same workspace, without claiming it is connected", async () => {
    ...
    // the jlfbot:// one (android/core Connection.kt).
    const credential = `jlf_pair_${"a".repeat(43)}`;
    const inviteUrl = `jlfbot://pair?address=${encodeURIComponent(origin)}&token=${credential}&name=fixture`;
    let healthRequests = 0;
    const fetcher = vi.fn(async (url: string | URL | Request, init?: RequestInit) => {
      ...
      if (address === `http://127.0.0.1:${options.port}/api/auth/pairing`) {
        ...
        // A server started without JLFBOT_PUBLIC_URL: it mints the credential but
        ...
        return Response.json({ code, url: null, expiresAt, credential, serverName: "fixture", hint: "set JLFBOT_PUBLIC_URL" });
      }
      expect(address).toMatch(/\/\.well-known\/jlfbot\/environment$/);
      expect(init?.method).not.toBe("POST");
    ...
    expect(requests).toContain(`http://127.0.0.1:${options.port}/.well-known/jlfbot/environment`);
    expect(requests).toContain(`${origin}/.well-known/jlfbot/environment`);
    expect(requests.indexOf(`${origin}/.well-known/jlfbot/environment`)).toBeLessThan(requests.indexOf(pairingRequest));
    ...
    expect(output).toContain("On Android, open the JLFBot app and scan the QR with its pairing scanner.");
