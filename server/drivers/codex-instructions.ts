...
export function codexDeveloperInstructions(config: unknown, botInstructions: string): string {
  ...
  return configured
    ? `${botInstructions || "No JLFBot bot-specific instructions remain."}\n\n${configured}`
    : botInstructions;
}
    ...
    try {
      await request("thread/inject_items", {
        ...
        items: [{
          ...
          content: [{
            type: "input_text",
            text: "The following replaces the previous developer instruction block supplied by JLFBot, including its native configured rules and bot-specific instructions. Other Codex instructions and permissions still apply.\n\n"
              + (instructions || "No JLFBot bot-specific instructions remain."),
          }],
        }],
      });
    } catch (error) {
