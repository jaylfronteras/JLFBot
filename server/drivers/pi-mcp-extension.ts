  ...
  async init(): Promise<void> {
    await this.call(
      "initialize",
      {
        ...
        capabilities: {},
        clientInfo: { name: "jlfbot-pi", version: "1" },
      },
...
export default async function (pi: PiExtensionApi): Promise<void> {
  const configPath = process.env.JLFBOT_MCP_CONFIG;
  ...
  const mounts = await Promise.all(
    serverEntries.map(async ([serverName, def]) => {
      ...
      try {
        ...
        process.stderr.write(`[jlfbot-pi-mcp] ${serverName}: ${message}\n`);
        client?.dispose();
        return { serverName };
      }
    }),
  ...
  for (const mount of mounts) {
    ...
    for (const tool of tools) {
      if (!tool || typeof tool.name !== "string" || !tool.name.trim()) {
        process.stderr.write(`[jlfbot-pi-mcp] ${serverName}: skipped a tool with no valid name\n`);
        continue;
      }
      ...
      try {
        ...
        const message = err instanceof Error ? err.message : String(err);
        process.stderr.write(`[jlfbot-pi-mcp] ${serverName}:${toolName}: ${message}\n`);
      }
    }
