...
export function codexLocalProviderArgs(
  ...
  if (!host) return [];
  const envKey = `JLFBOT_LOCAL_${host.id.toUpperCase().replace(/[^A-Z0-9]+/g, "_")}_API_KEY`;
...
export async function mergeLocalInject(
  ...
  const probe = env.JLFBOT_PROBE_LOCAL_INJECT ?? process.env.JLFBOT_PROBE_LOCAL_INJECT;
