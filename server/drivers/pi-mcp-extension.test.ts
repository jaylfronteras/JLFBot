...
const originalMcpConfig = process.env.JLFBOT_MCP_CONFIG;
...
function tempDir(): string {
  const dir = mkdtempSync(join(tmpdir(), "jlfbot-pi-mcp-ext-"));
  tempDirs.push(dir);
  return dir;
}
...
afterEach(() => {
  ...
  if (originalMcpConfig === undefined) delete process.env.JLFBOT_MCP_CONFIG;
  else process.env.JLFBOT_MCP_CONFIG = originalMcpConfig;
});
...
describe("Pi MCP extension registration", () => {
  it("keeps earlier tools alive when a later registration fails", async () => {
    ...
    process.env.JLFBOT_MCP_CONFIG = config;
