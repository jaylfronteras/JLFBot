...
const base: FleetInput = {
  ...
  node: "/usr/bin/node", script: "/usr/lib/node_modules/jlfbot/cli.js", root: "/",
};
const registryFile = "/etc/jlfbot/fleet.json";
const withRegistry = (workspaces = {}) => ({ [registryFile]: JSON.stringify({ ...emptyRegistry("agentada.cc"), workspaces }) });

describe("jlfbot fleet", () => {
  it("updates only managed OpenRouter models and rejects missing, malformed or redirected config", async () => {
    ...
    const initialFiles = { ...withRegistry({ acme: workspace }), [path]: JSON.stringify(config), [cfgPath]: '{"defaultModelSelection":{"instanceId":"claude","model":"keep"}}', "/etc/jlfbot/instances/acme.env": "JLFBOT_ADMIN_URL=https://admin.example.test\nJLFBOT_ADMIN_WORKSPACE=acme\n" };
    ...
    expect(m.calls).toEqual([`read ${path} as jlfbot-acme`, `write ${path} 600 as jlfbot-acme`]);
  ...
  it("prints the plan instead of acting when not root, and on --dry-run even as root", async () => {
    ...
    expect(out.join("\n")).toContain("cat > /etc/systemd/system/jlfbot@.service <<'JLFBOT_EOF'");
    expect(out.join("\n")).toContain("systemctl enable --now jlfbot-fence.service");
    ...
    expect(dry.out.join("\n")).toContain("useradd --system --create-home --home-dir /var/lib/jlfbot/acme");
  ...
  it("refuses an npx cache as the unit's script and asks for the domain", async () => {
    ...
    const bad = io();
    expect(await runFleetCommand({ ...base, action: "init", domain: "agentada.cc", script: "/root/.npm/_npx/abc/node_modules/jlfbot/cli.js" }, bad.io, deps)).toBe(2);
  ...
  it("edits a workspace's sign-in list in place, owned by the workspace, and lists what runs", async () => {
    ...
    expect(ready.calls).toContain(`read ${dataFile} as jlfbot-acme`);
    expect(ready.calls).toContain(`write ${dataFile} 600 as jlfbot-acme`);
  ...
  it("reinitializes templates without losing the operator, reservations, ports or fence", async () => {
    ...
    const prior = { ...emptyRegistry("agentada.cc"), operator: "jlf", nextPort: 8840, workspaces };
    ...
    expect(ready.files.get("/etc/jlfbot/fence.nft")).toBe(fenceRules(Object.values(workspaces)));
    expect(ready.files.get("/etc/systemd/system/jlfbot-fleet.service")).toContain("--group jlf");
    expect(ready.calls).toContain("run nft -f /etc/jlfbot/fence.nft");
  ...
  it.each(["/var/lib/jlfbot/acme", "/etc/jlfbot/instances/acme.env", "/etc/caddy/jlfbot.d/acme.caddy", "/etc/systemd/system/jlfbot@acme.service.d"])("refuses residual customer path %s before taking action", async (path) => {
  ...
  it.each(["provisioning", "error", "retained"])("lists a %s reservation accurately and rejects duplicate creation before residual paths", async (status) => {
    const ready = machine({ root: true, files: { ...withRegistry({ acme: { slug: "acme", host: "acme.agentada.cc", port: 8810, webhookPort: 8811, status, createdAt: "" } }), "/etc/jlfbot/instances/acme.env": "retained" } });
    const listed = io();
