...
describe("routine delegation through the isolated harness", () => {
  ...
  const delegate = async (threadId: string) => {
    ...
    const result = await api("POST", "/api/internal/delegate-bot", { toBotId: peer.id, message: "Produce the fixture report." }, launched.mcpConfig.mcpServers.agents.env.JLFBOT_COMMS_TOKEN);
  ...
  it("coordinates a new user request on a completed routine's thread without changing its recorded result", async () => {
    ...
    }, launched.mcpConfig.mcpServers.agents.env.JLFBOT_COMMS_TOKEN);
