...
it("keeps routing and secrets per instance and uses separate device-scoped Claude/Codex homes", () => {
  ...
  expect(codex.environment).toMatchObject({ JLFBOT_COMPANY_API_KEY: value.token });
  ...
  expect(router.config).toMatchObject({ url: `${value.portalOrigin}/api/desktop/gateway/openrouter/v1`, provider: "", apiKeyEnv: "JLFBOT_COMPANY_API_KEY" });
...
it("runs native Codex with Company Responses routing, isolated home and no OAuth or argv credential fallback", async () => {
  ...
  try {
    native = await CodexDriver.create({ instanceId: "fixture-company-codex", displayName: "Company", enabled: true, config,
      environment: { CODEX_HOME: isolatedHome, JLFBOT_COMPANY_API_KEY: value.token, OPENAI_API_KEY: "personal-must-not-leak", FAKE_CODEX_DUMP: dump, FAKE_CODEX_MODE: "logged-out" } });
    ...
    expect(spawned.env.JLFBOT_COMPANY_API_KEY).toBe(value.token);
    ...
    expect(JSON.stringify(spawned.argv)).not.toContain(value.token);
    expect(spawned.calls.find((call: { method: string }) => call.method === "thread/start").params).toMatchObject({ model: "gpt-fixture", modelProvider: "jlfbot_company" });
