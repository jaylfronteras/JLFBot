...
function apiBase() {
  return (process.env.JLFBOT_COMPOSIO_API ?? `${DEFAULT_BACKEND_ORIGIN}/api/v3.1`).replace(/\/$/, "");
}

function toolkitBase() {
  return (process.env.JLFBOT_COMPOSIO_TOOLKITS_API ?? `${DEFAULT_BACKEND_ORIGIN}/api/v3`).replace(/\/$/, "");
}
...
export function applyManagedBrokerMessage(message: unknown): boolean {
  ...
  if (
    !parsed.success ||
    parsed.data.type !== "jlfbot:managed-composio" ||
...
function brokerAccess(): { url: string; token: string } | null {
  ...
  const url = process.env.JLFBOT_COMPOSIO_BROKER_URL?.trim();
  const token = process.env.JLFBOT_COMPOSIO_BROKER_TOKEN?.trim();
...
/** Three answers, not two. The desktop shell sets JLFBOT_CREDENTIAL_STORE to
...
export function connectorAvailability(
  cfg: AppConfig,
  storeState: string | undefined = process.env.JLFBOT_CREDENTIAL_STORE,
...
export async function prepareProjectSession(
  ...
  if (trimmed === current?.apiKey && current.sessionId) {
    ...
    if (
      ...
      return {
        apiKey: trimmed,
        userId: existing.config?.user_id ?? current.userId ?? `jlfbot_${randomUUID()}`,
  ...
  const userId = priorUserId ?? `jlfbot_${randomUUID()}`;
...
export async function mcpIntegration(
  ...
  return {
    ...
    env: {
      ...
      JLFBOT_CONNECTOR_UPSTREAM_URL: `${context.harnessUrl}/api/internal/connectors/mcp`,
      JLFBOT_CONNECTOR_UPSTREAM_HEADERS: JSON.stringify({ authorization: `Bearer ${context.commsToken}` }),
      JLFBOT_HARNESS_URL: context.harnessUrl,
      ...
      JLFBOT_CONNECTOR_TOKEN: context.commsToken,
      JLFBOT_BOT_ID: context.botId,
      JLFBOT_THREAD_ID: context.threadId,
    },
  };
}
