...
const EXCLUDES = `# JLFBot checkpoint excludes — never snapshotted, never removed by restore
.git/
.svn/
.hg/
node_modules/
...
function gitEnv(shadow: string, cwd: string): NodeJS.ProcessEnv {
  ...
  env.GIT_AUTHOR_NAME = "JLFBot Checkpoint";
  env.GIT_AUTHOR_EMAIL = "checkpoint@jlfbot.local";
  env.GIT_COMMITTER_NAME = "JLFBot Checkpoint";
  env.GIT_COMMITTER_EMAIL = "checkpoint@jlfbot.local";
  return env;
}
