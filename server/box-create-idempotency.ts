...
export interface BoxCreateRequest {
  ...
  createdAt: number;
  boxId?: string;
  /** The provider Box has its deterministic JLFBot name. Until this is
...
function recoveryStateError(detail: string, cause?: unknown): Error & { status: number } {
  return Object.assign(
    new Error(
      ...
      + "Check ascii.dev for an unnamed Box before repairing JLFBot's local state.",
    ),
    { status: 503, cause },
  );
}
  ...
  try {
    for (;;) {
      ...
      if (performance.now() >= deadline) {
        throw recoveryStateError("locked by another JLFBot process");
      }
      if (reaped) continue;
