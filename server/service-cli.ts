// The `jlfbot service` command, kept separate from cli.ts so it can be
...
export function runServiceCommand(input: ServiceInstallInput, io: ServiceIo): number {
  ...
  if (!plan) {
    io.error("services are written for Linux (systemd) and macOS (launchd); on Windows, use Task Scheduler to run `jlfbot serve` at startup");
    return 1;
  }
  ...
  io.log(`logs: ${platform === "darwin" ? `${input.dataDir}/logs/service.log` : "journalctl -u jlfbot -f"}`);
  io.log(`change options later by running \`jlfbot service install\` again with the new ones, then: ${platform === "darwin" ? plan.activate[1] : "sudo systemctl daemon-reload && sudo systemctl restart jlfbot"}`);
  return 0;
}
