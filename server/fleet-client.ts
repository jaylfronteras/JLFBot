...
export function fleetSocketPath(env: NodeJS.ProcessEnv = process.env): string {
  return env.JLFBOT_FLEET_SOCKET?.trim() || "/run/jlfbot/fleet.sock";
}
  ...
  return new Promise((resolve, reject) => {
    ...
    req.on("error", (error: NodeJS.ErrnoException) => {
      reject(new Error(error.code === "ENOENT" || error.code === "ECONNREFUSED"
        ? "no fleet agent on this server: run `jlfbot fleet init --domain … --operator <this user>` as root"
        : error.code === "EACCES"
          ? "this workspace's user may not open the fleet socket: re-run `jlfbot fleet init` with --operator set to it"
