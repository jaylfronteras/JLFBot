...
export const LICENSE_EXPIRED_MESSAGE = "Your organisation's JLFBot Admin licence has expired. Contact your admin.";
...
export function companyInstanceConfigs(connection: ManagedDesktopConnection, runtimeDirectory: string): InstanceConfigMap {
  ...
  for (const provider of connection.providers) {
    ...
    if (provider.id === "openai") entries[id] = {
      ...
      environment: { JLFBOT_COMPANY_API_KEY: connection.token, CODEX_HOME: join(runtimeDirectory, id, "codex") },
    };
    if (provider.id === "openrouter") entries[id] = {
      driver: "openai-compat", displayName, config: { url: `${base}/v1`, apiKeyEnv: "JLFBOT_COMPANY_API_KEY", model: provider.models[0], provider: "" },
      environment: { JLFBOT_COMPANY_API_KEY: connection.token },
    };
  }
  return entries;
}
