...
afterEach(() => {
  ...
  delete process.env.JLFBOT_TOKEN;
...
describe("MCP JSON-RPC protocol", () => {
  it("negotiates supported and newer protocol versions", async () => {
    ...
    expect(supported.result).toMatchObject({
      ...
      serverInfo: { name: "jlfbot-mcp", version: "1.1.0" },
      capabilities: { tools: {} },
    });
...
describe("connection security and discovery", () => {
  it("accepts loopback HTTP and HTTPS origins, but rejects unsafe URL shapes", () => {
    ...
    expect(validateBaseUrl("https://jlf.example.com")).toBe("https://jlf.example.com");
    expect(() => validateBaseUrl("ftp://jlf.example.com")).toThrow("http:// or https://");
    expect(() => validateBaseUrl("https://jlf.example.com/api")).toThrow("origin without a path");
    expect(() => validateBaseUrl("https://user:pass@jlf.example.com")).toThrow("must not contain credentials");
  ...
  it("rejects successful non-JSON responses and sends an optional bearer token", async () => {
    process.env.JLFBOT_TOKEN = "proxy-token";
    ...
    await expect(request("/api/health", {}, "https://jlf.example.com")).rejects.toThrow("non-JSON response");
  });
  ...
  it("requires an explicit destination before sending a bearer token", async () => {
    process.env.JLFBOT_TOKEN = "proxy-token";
    await expect(resolveBaseUrl()).rejects.toThrow("JLFBOT_URL or JLFBOT_PORT");
  });
