...
describe("chiefOfStaffSystemPrompt", () => {
  ...
  it("includes trusted JLFBot status only when the Chief caller supplies it", () => {
    const status = "TRUSTED JLFBOT STATUS\nfreshness=fresh; runtime_state=degraded";
    ...
    expect(ordinaryPrompt).not.toContain("TRUSTED JLFBOT STATUS");
  });

  it("pins legacy delegation guidance and shared staffing independently of the bounded chat coordinator", () => {
    const prompt = chiefOfStaffSystemPrompt("chief", bots, true, "TRUSTED JLFBOT STATUS\nfreshness=fresh");
