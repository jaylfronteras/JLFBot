...
if (!podman || !machine) throw new Error("Set JLFBOT_VERIFY_PODMAN and JLFBOT_VERIFY_MACHINE to an explicit test engine");
const output = resolve(process.env.JLFBOT_VERIFY_OUTPUT || ".jlfbot-scratch/firefox");
...
process.env.JLFBOT_DATA_DIR = resolve(fixtureHome, "app-data");
...
for (const mode of ["before", "after"] as const) {
  const workspace = execFileSync(podman, ["machine", "ssh", machine, "mktemp", "-d", "/tmp/jlfbot-firefox-XXXXXXXX"], { env: engineEnv, encoding: "utf8", timeout: 30_000 }).trim();
  assert.match(workspace, /^\/tmp\/jlfbot-firefox-[A-Za-z0-9]+$/);
