...
describe("CI concurrency", () => {
  it("supersedes old main and PR checks", () => {
  ...
  it("keeps each ref in its own concurrency group", () => {
  ...
  it("allows cancelled summary jobs to stop without skipping failure reporting", () => {
    ...
    expect(workflow.jobs.gate.needs).toEqual([
      "static", "vitest", "packaged-server", "windows-cua", "electron-smokes", "package-linux",
    ]);
  });
