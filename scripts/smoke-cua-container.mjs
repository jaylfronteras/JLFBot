...
const runtime = process.env.JLFBOT_CUA_RUNTIME || "docker";
const container = process.env.JLFBOT_CUA_CONTAINER;
const socket = process.env.JLFBOT_CUA_SOCKET || "/run/user/1000/jlfbot-cua.sock";
if (!container || !/^[a-zA-Z0-9_.-]+$/.test(container)) {
  throw new Error("set JLFBOT_CUA_CONTAINER to the explicitly created smoke container name");
}
...
try {
  await rpc("initialize", {
    ...
    capabilities: {},
    clientInfo: { name: "jlfbot-container-smoke", version: "1" },
  });
