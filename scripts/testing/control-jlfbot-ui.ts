// `control-jlfbot ui`: drive the real React renderer headlessly against the
...
//
// Imported by scripts/control-jlfbot.ts, which owns HELP and the MUTATING set;
...
export const UI_TOOLS_DIR = join(ROOT, ".jlfbot-scratch", "verify-tools");
...
const ENTRIES = {
  threads: { entry: "/scripts/testing/threads-preview.tsx", route: "/__threads.html", title: "Isolated JLFBot Chat" },
...
/** JLFBOT_AGENT_BROWSER_PATH, then the tools directory, then PATH; otherwise the
...
export async function ensureUiBrowser(
  ...
  if (!chrome) {
    const started = Date.now();
    await ensureChrome(binary, { env: sessionEnv({ home: toolsDir, session: "jlfbot-ui-install", chrome: null }, parentEnv), log });
...
export async function runControlOmbUi(args: string[]): Promise<unknown> {
  ...
  if (verb === "launch") {
    throw new ControlOmbError("ui launch is available only from the executable CLI", "run `node --experimental-strip-types scripts/control-jlfbot.ts ui launch`");
  }
  ...
  throw new ControlOmbError(`unknown ui command ${JSON.stringify(verb)}`, "run control-jlfbot ui help");
}
