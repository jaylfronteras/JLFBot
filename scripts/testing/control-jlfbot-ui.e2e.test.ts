...
// machine. It spawns the real `control-jlfbot ui launch` (a child it can Ctrl-C),
...
if (!enabled) {
  console.log(`skipping control-jlfbot ui e2e: no agent-browser binary resolves (looked in ${UI_TOOLS_DIR}, JLFBOT_AGENT_BROWSER_PATH and PATH); set JLFBOT_UI_E2E=1 to install the pinned release`);
}
...
const TOOL_CALLS = JSON.stringify([
  { name: "Bash", input: { command: "pnpm control:jlfbot doctor" }, ok: true },
  { name: "Bash", input: { command: "pnpm control:jlfbot ui click --name Missing" }, ok: false },
  { name: "Bash", input: { command: "pnpm control:jlfbot ui flag --set features.showToolCalls=true --dry-run" }, ok: true },
]);
...
describe("control-jlfbot ui drives the real renderer", () => {
  ...
  run("sends a turn from the composer and shows the reply and the scripted tool chip", async () => {
    ...
    expect(handle).toMatchObject({ url: info.url, previewUrl: info.previewUrl, botId: info.botId, home: info.dataDir, session: `jlfbot-ui-${new URL(info.url).port}` });
    ...
    expect(draft).toContain("✓ doctor — pnpm control:jlfbot doctor (verified)\n");
    expect(draft).toContain("✗ ui — pnpm control:jlfbot ui click --name Missing (verified)\n");
    expect(draft).toContain("[dry run] ui — pnpm control:jlfbot ui flag --set features.showToolCalls=true --dry-run (verified)\n");
