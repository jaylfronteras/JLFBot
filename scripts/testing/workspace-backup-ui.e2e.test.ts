...
if (!enabled) console.log("skipping workspace backup UI e2e: no agent-browser; set JLFBOT_UI_E2E=1 to install the pinned release");
  ...
  (enabled ? it : it.skip)("uses passwords, a file preview and explicit replacement without leaking secrets", async () => {
    ...
    await evaluate(`(() => {
      ...
      window.fetch = (input, init = {}) => {
        ...
        if (path.endsWith('/export')) return reply({ id: 'download-stage', filename: 'fixture.jlfbotbackup' });
      ...
      localStorage.setItem('jlfbot-drafts', JSON.stringify({fixture: 'private fixture draft'}));
      localStorage.setItem('fixture-auth-token', 'must not export');
      localStorage.setItem('jlfbot-webhook-credentials', 'fixture private URL must not export');
      return true;
    })()`);
    ...
    await ui("screenshot", "--out", join(ROOT, ".jlfbot-scratch", "verify-evidence", "workspace-backup-settings.png"));
    ...
    await expect.poll(() => evaluate("window.backupFixture.download"), { timeout: 10_000 }).toEqual({ href: "/api/workspace-backup/download/download-stage", filename: "fixture.jlfbotbackup" });
    ...
    expect(await evaluate("window.backupFixture.calls.find(call => call.path.endsWith('/export')).body.clientState['jlfbot-webhook-credentials'] ?? null")).toBeNull();
