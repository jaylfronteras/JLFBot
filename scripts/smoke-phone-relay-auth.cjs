...
const home = mkdtempSync(join(tmpdir(), "jlfbot-phone-auth-smoke-"));
...
app.whenReady().then(async () => {
  ...
  const harness = fork("dist-server/index.js", { ...env, JLFBOT_DESKTOP_PARENT: "1" });
  await until(() => api(harnessPort, "/api/health").catch(() => null));
  assert.equal((await api(harnessPort, "/api/bots", "POST", {})).status, 403);
  harness.postMessage({ type: "jlfbot:desktop-mutation-token", token: owner, companionToken: relay });
  ...
  assert.equal((await api(harnessPort, "/api/bots", "POST", {}, {
    "x-jlfbot-companion": "1", "x-jlfbot-companion-device": paired.body.device.id,
  })).status, 403);
  sidecar.postMessage({ type: "jlfbot:companion-mutation-token", token: relay });
  ...
  const created = await phone("/api/bots", "POST", { modelSelection: { instanceId: "claude", model: "claude-sonnet-5" } }, {
    "x-jlfbot-companion-auth": "forged", "x-jlfbot-desktop-owner": "forged",
    "x-jlfbot-companion-device": "forged-device",
  });
  ...
  harness.postMessage({ type: "jlfbot:desktop-mutation-token", token: owner, companionToken: nextRelay });
  const restarted = fork("dist-companion/index.js", env);
  restarted.once("spawn", () => restarted.postMessage({ type: "jlfbot:companion-mutation-token", token: nextRelay }));
  ...
  const evidence = join(tmpdir(), `jlfbot-phone-auth-${Date.now()}.log`);
