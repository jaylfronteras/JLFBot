 ...
 * whitespace — never inside a token — matching how `JlfSilhouette.path` in
 * `ios/App/JlfAvatar.swift` is already formatted. That parser treats newlines as plain
...
// \`JlfSilhouette\` in \`ios/App/JlfAvatar.swift\`, which treats newlines as separators.
import CoreGraphics
 ...
 *
 * Plain Kotlin, no Compose: the data is parsed into a `Path` by `JlfSilhouette`, which
...
function emitKotlin(bodies: Baked[]): string {
  ...
  return `package com.jlfbot.companion.ui

/**
 * The mascot bodies a bot can wear — the Android half of the same solve that bakes
 * \`shared/mascot-bodies.ts\` and \`ios/Sources/CompanionCore/JlfBodies.swift\`.
 *
 * GENERATED FILE — do not hand-edit. Run \`pnpm gen:bodies\` to rebuild it from
 * \`scripts/gen-mascot-bodies.ts\`, which solves each face placement against the real
 ...
 *
 * Plain Kotlin, no Compose: [JlfSilhouette] parses the path and caches the result per
...
function main(): void {
  ...
  const swiftOut = fileURLToPath(
    new URL("../ios/Sources/CompanionCore/JlfBodies.swift", import.meta.url)
  )
  ...
  const kotlinOut = fileURLToPath(
    new URL("../android/app/src/main/kotlin/com/jlfbot/companion/ui/JlfBodies.kt", import.meta.url)
  )
