...
const fixtureFlag = "--jlfbot-server-connection-fixture";
...
if (process.versions.electron && process.argv.includes(fixtureFlag)) {
    ...
    const open = async (local) => {
      const win = new BrowserWindow({
        ...
        webPreferences: {
          ...
          additionalArguments: [`--jlfbot-local-origin=${local ? new URL(url).origin : "https://remote-fixture.invalid"}`],
        },
      });
  ...
  const output = mkdtempSync(join(tmpdir(), "jlfbot-server-connection-"));
