...
function translationPrompt(source, label, code) {
  return [
    `Translate this JSON UI catalog for JLFBot, a multi-agent desktop workbench, into ${label} (${code}).`,
    ...
    "Keep placeholders such as {name} exactly, including duplicates. Keep JLFBot, CLI, and AI unchanged.",
...
function runModel(prompt) {
  const workDir = mkdtempSync(join(tmpdir(), "jlfbot-locale-"));
  try {
