...
// JLFBot updates this dependency with an ordinary reviewed app release.
...
export function parsePrepareCloudflaredArgs(args = []) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (argument === "--current" && !options.current) {
      ...
      // `jlfbot serve --tunnel` stages into its data dir, not a checkout.
      options.root = next;
      index += 1;
    } else {
...
export async function releaseBytes(asset) {
  const cacheDirectory = process.env.JLFBOT_CLOUDFLARED_ARCHIVE_DIR;
...
async function stageTarget(root, target) {
  ...
  const scratch = mkdtempSync(join(tmpdir(), `jlfbot-cloudflared-${target}-`));
  try {
