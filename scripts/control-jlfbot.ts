...
import { launchUi, runControlOmbUi } from "./testing/control-jlfbot-ui.ts";
...
export const HELP_UI = `renderer (needs a ui launch handle; every verb takes --ui HANDLE, never discovery):
  node --experimental-strip-types scripts/control-jlfbot.ts ui launch [--entry threads] [--tool-calls JSON] [--mode happy]
...
mutating (an explicit --url or JLFBOT_URL/JLFBOT_PORT is required):
...
export function parse(
  ...
  try {
    ...
    throw new ControlOmbError(
      ...
      `run control-jlfbot help for the ${command} syntax`,
    );
  }
}
...
function configuredUrl(raw: unknown, env: NodeJS.ProcessEnv, requiredForMutation: boolean): string | undefined {
  const explicit = typeof raw === "string" && raw.trim()
    ? raw.trim()
    : env.JLFBOT_URL?.trim() || (env.JLFBOT_PORT ? `http://127.0.0.1:${env.JLFBOT_PORT}` : "");
  if (!explicit) {
...
export async function runControlOmb(
  ...
  throw new ControlOmbError(`unknown command ${JSON.stringify(command)}`, "run control-jlfbot help");
}
...
export async function launchVerificationServer(
  ...
  const dataDir = mkdtempSync(join(browser && process.platform !== "win32" ? "/tmp" : tmpdir(), "jlfbot-verify-data-"));
  const fixtureTemp = join(dataDir, "tmp");
  const fixtureDumpPath = join(dataDir, "fake-claude-dump.json");
  mkdirSync(fixtureTemp, { recursive: true });
  const evidenceDir = join(tmpdir(), "jlfbot-verification-evidence");
  ...
  if (localVm) Object.assign(childEnv, {
    JLFBOT_EXTRA_PATH: [localVm.binDir, ...(process.platform === "win32" ? [join(childEnv.SYSTEMROOT || "C:\\Windows", "System32")] : [])].join(delimiter),
    CONTAINER_HOST: localVm.host,
    CONTAINER_SSHKEY: localVm.sshKey,
    JLFBOT_STATIC_DIR: localVm.staticDir,
  });
  if (enterprise) Object.assign(childEnv, { JLFBOT_ENTERPRISE_DIR: enterprise.dir, JLFBOT_LICENSE_KEY: enterprise.licenseKey });
  if (browser) Object.assign(childEnv, {
    JLFBOT_AGENT_BROWSER_PATH: browser.binaryPath,
    AGENT_BROWSER_EXECUTABLE_PATH: browser.executablePath,
  });
  if (boxFixtureApi) childEnv.JLFBOT_BOX_API = boxFixtureApi;
...
function requireForegroundTerminal(command: string): void {
  if (process.env.npm_lifecycle_event === "control:jlfbot") {
    throw new ControlOmbError(
      `${command} must own the terminal directly so Ctrl-C can clean up its children`,
      `run \`node --experimental-strip-types scripts/control-jlfbot.ts ${command}\``,
    );
  }
}
