...
describe("desktop notifications", () => {
  ...
  it("shows a notification after permission is granted", () => {
    ...
    expect(notices[0]).toMatchObject({ title: frame.title, options: { body: frame.body, tag: `jlfbot:${frame.botId}` } });
  });
  ...
  it("shows a spend notice even over the thread it points at, in its own stack", () => {
    ...
    expect(notices).toHaveLength(1);
    expect(notices[0]!.options).toMatchObject({ tag: "jlfbot:spend", icon: undefined });
  });
  ...
  it("groups under the bot, not the thread", () => {
    ...
    expect(notices[0]?.options?.tag).toBe(`jlfbot:${frame.botId}`);
    expect(notices[1]?.options?.tag).toBe(`jlfbot:${frame.botId}`);
    showNotification({ ...frame, botId: "bot-2" }, vi.fn());
    expect(notices[2]?.options?.tag).toBe(`jlfbot:bot-2`);
  });
