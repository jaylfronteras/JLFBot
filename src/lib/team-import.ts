...
export function teamImportPreview(manifest: unknown): PendingTeamImport {
  ...
  if (root.format === "jlfbot.package") return packagePreview(root, manifest);
  if (root.format !== "jlfbot.team") throw new Error("This is not an JLFBot backup, BotMRR playbook or legacy team.");
...
function markdownPackage(markdown: string): unknown {
  ...
  return { format: "jlfbot.package", version: 1, package: pkg };
}
