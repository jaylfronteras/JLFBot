...
describe("companionPairingLink", () => {
  const token = `jlf_pair_${"a".repeat(43)}`;
  ...
  it("carries the dialable address, one-time token, fallback code, and display name", () => {
    ...
    expect(url.protocol).toBe("jlfbot:");
  ...
  it("carries the ordered fallback hosts, comma-joined", () => {
    ...
    expect(new URL(link!).searchParams.get("hosts")).toBe(
      "macbook.tail1234.ts.net,192.168.1.42,jlfbot-abcd1234.local",
    );
  });
  ...
  it("carries sorted typed endpoints as URL-safe base64 JSON while preserving legacy fields", () => {
    const link = companionPairingLink({
      ...
      code: "004209",
      token,
      hosts: ["192.168.1.42", "jlfbot-abcd1234.local"],
      endpoints: [
    ...
    expect(url.searchParams.get("hosts")).toBe("192.168.1.42,jlfbot-abcd1234.local");
  ...
  it("makes the automatic QR hosted-only even when Tailscale and LAN are advertised", () => {
    const endpoints = [
      { url: "https://device.jlfbot.example.com", kind: "hosted" as const, priority: 0 },
    ...
    expect(route).toEqual({
      address: "device.jlfbot.example.com",
      port: 443,
      hosts: ["device.jlfbot.example.com"],
      endpoints: [endpoints[0]],
    });
    const link = companionPairingLink({ ...route!, code: "004209", token });
    const url = new URL(link!);
    expect(url.searchParams.get("address")).toBe("device.jlfbot.example.com:443");
    expect(url.searchParams.get("hosts")).toBe("device.jlfbot.example.com");
  ...
  it("pins the protected automatic transport instead of downgrading the live QR to LAN", () => {
    ...
    expect(pin?.protectedEndpoint).toEqual({
      url: "https://device.jlfbot.example.com",
      kind: "hosted",
      priority: 0,
    });
  ...
  it("selects hosted HTTPS regardless of an unprotected endpoint's priority", () => {
    expect(companionPairingRoutePin({
      ...
      endpoints: [
        ...
        { url: "https://device.jlfbot.example.com", kind: "hosted", priority: 100 },
      ],
  ...
  it("makes the explicitly selected LAN route first without losing protected upgrades", () => {
    const route = companionPairingRoute({
      ...
      hosts: ["mac.tail1234.ts.net", "192.168.1.42", "jlfbot-aa.local"],
      endpoints: [
        { url: "https://device.jlfbot.example.com", kind: "hosted", priority: 0 },
        { url: "http://mac.tail1234.ts.net:8810", kind: "tailnet", priority: 100 },
        { url: "http://192.168.1.42:8810", kind: "lan", priority: 200 },
        { url: "http://jlfbot-aa.local:8810", kind: "bonjour", priority: 300 },
      ],
    }, "local");
    ...
    expect(decodedEndpoints(link!)).toEqual([
      { url: "http://192.168.1.42:8810", kind: "lan", priority: 0 },
      { url: "https://device.jlfbot.example.com", kind: "hosted", priority: 100 },
      { url: "http://jlfbot-aa.local:8810", kind: "bonjour", priority: 200 },
    ]);
  });
  ...
  it("uses an advertised Bonjour route when no LAN address is available", () => {
    const route = companionPairingRoute({
      port: 8810,
      hosts: ["mac.tail1234.ts.net", "jlfbot-aa.local"],
      discovery: { advertising: true, name: "jlfbot-aa.local" },
      endpoints: [
        { url: "https://device.jlfbot.example.com", kind: "hosted", priority: 0 },
        { url: "http://jlfbot-aa.local:8810", kind: "bonjour", priority: 300 },
      ],
    ...
    expect(route?.address).toBe("jlfbot-aa.local");
    expect(route?.hosts?.[0]).toBe("jlfbot-aa.local");
  ...
  it("does not treat an inactive synthetic Bonjour name as a reachable local route", () => {
    expect(companionPairingRoute({
      port: 8810,
      hosts: ["mac.tail1234.ts.net", "jlfbot-aa.local"],
      discovery: { advertising: false, name: "jlfbot-aa.local" },
      endpoints: [
        { url: "https://device.jlfbot.example.com", kind: "hosted", priority: 0 },
        { url: "http://jlfbot-aa.local:8810", kind: "bonjour", priority: 300 },
      ],
