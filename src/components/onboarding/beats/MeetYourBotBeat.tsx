...
import { JLF_COLOR_NAMES, JLF_COLORS, type JlfColor } from "@/lib/mascot";
...
export function MeetYourBotBeat({
  ...
  const [color, setColor] = useState<JlfColor>(bot?.color ?? "green");
  ...
  return (
    <div className="stagger flex flex-col">
      ...
      <div className="animate-rise mt-5 flex items-center gap-4" style={staggerIndex(1)}>
        ...
        <JlfAvatar color={color} state="happy" size={64} label={name || bot?.name} />
        ...
        <div role="radiogroup" aria-label={t("onboarding.bot.color")} className="mt-2 flex flex-wrap gap-2">
          {JLF_COLOR_NAMES.map((c) => (
            <button
              ...
              style={{ backgroundColor: JLF_COLORS[c] }}
            />
          ))}
        </div>
      </div>
