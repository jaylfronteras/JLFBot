...
export function managedComposioChildEnvironment(brokerUrl, credentials, environment) {
  ...
  delete next.JLFBOT_COMPOSIO_BROKER_URL;
  delete next.JLFBOT_COMPOSIO_BROKER_TOKEN;
  const access = managedComposioAccess(brokerUrl, credentials);
  if (access) {
    next.JLFBOT_COMPOSIO_BROKER_URL = access.url;
    next.JLFBOT_COMPOSIO_BROKER_TOKEN = access.token;
  }
  return next;
}
