...
const DEVICE_TOKEN = /^jlf_[A-Za-z0-9_-]{43}$/;
...
export function desktopCompanionRendererArguments(localOrigin, remoteAccess) {
  return [
    `--jlfbot-local-origin=${localOrigin}`,
    ...(remoteAccess ? ["--jlfbot-remote-client"] : []),
  ];
}
...
/** Pairing tokens may travel through a verified JLFBot HTTPS endpoint or
...
export function normalizeDesktopCompanionEndpoint(value) {
  ...
  const managedHttps =
    parsed.protocol === "https:" && hostname.endsWith(".jlfbot.example.com");
  if (
...
export async function pairDesktopCompanion({
  ...
  if (!endpoint) {
    throw new Error("Enter the JLFBot HTTPS companion address or full Tailscale name ending in .ts.net");
  }
