...
export function resolveCloudflaredBinary({
  ...
  const override = environment.JLFBOT_CLOUDFLARED_PATH?.trim();
...
/** Remove only token files created by a dead JLFBot process. Suspicious
...
async function verifyHostedEndpoint(
  ...
  try {
    return JSON.parse(text)?.app === "jlfbot";
  } catch {
    return false;
  }
}
