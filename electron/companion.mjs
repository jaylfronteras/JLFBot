...
// sidecar's ~/.jlfbot-companion, which is the child process's directory,
...
async function start({ resourcesPath, harnessPort, mutationToken, hostedUrl = null, secretPublicKey = null, log }) {
  ...
  delete childEnvironment.JLFBOT_COMPANION_HOSTED_URL;
  delete childEnvironment.JLFBOT_COMPANION_INTERNAL_ORIGIN;
  delete childEnvironment.JLFBOT_PHONE_SECRET_PUBLIC_KEY;
  if (hostedUrl) childEnvironment.JLFBOT_COMPANION_HOSTED_URL = hostedUrl;
  childEnvironment.JLFBOT_COMPANION_INTERNAL_ORIGIN = allocatedOrigin.socketPath;
  if (/^[A-Za-z0-9_-]{87}$/.test(String(secretPublicKey ?? ""))) {
    childEnvironment.JLFBOT_PHONE_SECRET_PUBLIC_KEY = secretPublicKey;
  }
  ...
  try {
    child = utilityProcess.fork(resolved.entry, [], {
      env: {
        ...childEnvironment,
        JLFBOT_PORT: String(harnessPort),
        JLFBOT_COMPANION_PORT: String(COMPANION_PORT),
        JLFBOT_CONTROL_PORT: String(CONTROL_PORT),
      },
  ...
  child.once("spawn", () => {
    ...
    try {
      child.postMessage({ type: "jlfbot:companion-mutation-token", token: mutationToken });
    } catch {
 ...
 * phones. Tailscale may be installed, signed in, or enabled after JLFBot
...
export async function companionPairing(open, expectedToken) {
  ...
  const token = /^jlf_pair_[A-Za-z0-9_-]{43}$/.test(candidate)
    ? candidate
